...
## 自由本文（sections）でコンテンツを増やす

ランキング表・グラフの下に、**見出し(h2/h3)・段落・箇条書き・表を好きな順で好きなだけ**並べて、
普通の記事のように内容を厚くできる。記事ファイルに `sections` 配列を足すだけ（省略可）。
...
```json
{
  ...
  "sections": [
    { "h2": "大見出し" },
    { "p": "段落テキスト。文中で **ここ** と書くと太字になる。" },
    { "h3": "小見出し" },
    { "list": ["箇条書き1", "箇条書き2"] },
    { "table": {
      "headers": ["年産", "新潟県", "北海道"],
      "rows": [
        ["2023年産", "591,700", "540,200"],
        ["2024年産", "622,800", "562,400"]
      ],
      "caption": "単位: トン。出典: …（任意）"
    } }
  ]
...
- ブロックは上から書いた順に表示される。`h2` `h3` `p` `list` `table` を何個でも混ぜてよい
- 表の数値は**文字列**で書く（カンマ区切りOK。`"622,800"`）。セル内も `**太字**` が使える
- 使えないキーを書くとビルドが**エラーで止まる**（タイポ検出）。`table` は `rows`(配列)が必須
- HTMLタグは書いても**文字として表示される**（エスケープされる）。安全のための仕様
- 表示位置は「ランキング表・推移グラフ」の下、「考察」の上。ダークモードにも自動対応
