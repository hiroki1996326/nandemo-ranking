// このファイルは scripts/build.py が content/ から自動生成します。
// 直接編集しないでください（content/ を編集して再ビルドしてください）。
window.RANKING_DATA = {
  'categories': [
    {
      'id': 'keizai',
      'name': '経済・産業',
    },
    {
      'id': 'jinko',
      'name': '人口・社会',
    },
    {
      'id': 'chiri',
      'name': '地理・自然',
    },
    {
      'id': 'sports',
      'name': 'スポーツ・興行',
    },
  ],
  'topics': [
    {
      'id': 'gyokakuryo-todofuken',
      'title': '都道府県別 漁獲量ランキング',
      'category': 'keizai',
      'unit': 'トン',
      'source': '農林水産省 海面漁業生産統計調査',
      'sourceUrl': 'https://www.maff.go.jp/j/tokei/kouhyou/kaimen_gyosei/',
      'updateFrequency': 'yearly',
      'updatedAt': '2024-01-01',
      'lead': '2023年の海面漁業漁獲量が最も多いのは北海道で842,704トン。2位長崎県（292,890トン）の約2.9倍と、全国シェアの3割近くを占める。',
      'commentary': '北海道が単独で突出しており、2位以下は太平洋側・日本海側の県が入り交じる。',
      'analysisHeading': '北海道が単独で突出、2位以下は30万トンを下回る',
      'analysis': [
        '北海道の漁獲量は842,704トンで、2位長崎県（292,890トン）の約2.9倍。3位茨城県（259,496トン）まで含めた上位3道県で、10位愛媛県（74,096トン）までの合計のおよそ7割を占める。',
        '4位宮城県（180,369トン）以下は10万〜18万トン台で並ぶ。上位10道県の内訳は、太平洋側が北海道・茨城・宮城・静岡・千葉の5道県、日本海・東シナ海側が長崎・島根・鳥取の3県、宮崎・愛媛が続く。',
      ],
      'sections': [],
      'periods': [
        {
          'period': 2022,
          'entries': [
            {
              'name': '北海道',
              'value': 850981,
              'note': '広い海域を持ち、都道府県別漁獲量で全国1位。2023年漁業センサスによると、北海道の漁船隻数は17,068隻で全国の総隻数（109,283隻）の**約15.6%**を占め、隻数でも都道府県別で全国最多。',
            },
            {
              'name': '茨城県',
              'value': 285164,
              'note': '太平洋に面した漁場を持ち、漁獲量は全国3位。2023年の漁獲量259,496トンのうち「**いわし類**」が233,727トン（**約90%**）を占め、単一魚種への集中度が際立つ。いわし類の漁獲量自体も全国シェア24.7%で全国1位。',
            },
            {
              'name': '長崎県',
              'value': 262233,
              'note': '複雑な海岸線を持つ好漁場が多く、漁獲量は全国2位。水揚げされる魚種は**300種以上**で全国最多とされ、2019年にはマアジ（全国シェア42.1%）・サバ類（同15.9%）がいずれも漁獲量日本一だった。',
            },
          ],
        },
        {
          'period': 2023,
          'entries': [
            {
              'name': '北海道',
              'value': 842704,
              'note': '広い海域を持ち、都道府県別漁獲量で全国1位。2023年漁業センサスによると、北海道の漁船隻数は17,068隻で全国の総隻数（109,283隻）の**約15.6%**を占め、隻数でも都道府県別で全国最多。',
            },
            {
              'name': '長崎県',
              'value': 292890,
              'note': '複雑な海岸線を持つ好漁場が多く、漁獲量は全国2位。水揚げされる魚種は**300種以上**で全国最多とされ、2019年にはマアジ（全国シェア42.1%）・サバ類（同15.9%）がいずれも漁獲量日本一だった。',
            },
            {
              'name': '茨城県',
              'value': 259496,
              'note': '太平洋に面した漁場を持ち、漁獲量は全国3位。2023年の漁獲量259,496トンのうち「**いわし類**」が233,727トン（**約90%**）を占め、単一魚種への集中度が際立つ。いわし類の漁獲量自体も全国シェア24.7%で全国1位。',
            },
            {
              'name': '宮城県',
              'value': 180369,
            },
            {
              'name': '静岡県',
              'value': 144710,
            },
            {
              'name': '島根県',
              'value': 102252,
            },
            {
              'name': '鳥取県',
              'value': 100501,
            },
            {
              'name': '宮崎県',
              'value': 82611,
            },
            {
              'name': '千葉県',
              'value': 79158,
            },
            {
              'name': '愛媛県',
              'value': 74096,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/gyokakuryo-todofuken.webp',
    },
    {
      'id': 'jinko-todofuken',
      'title': '都道府県別 人口ランキング',
      'category': 'jinko',
      'unit': '人',
      'source': '総務省統計局 人口推計',
      'sourceUrl': 'https://www.stat.go.jp/',
      'updateFrequency': 'yearly',
      'updatedAt': '2025-10-01',
      'lead': '2025年10月1日時点の推計人口で最も多いのは東京都（約1,424万人）。2位神奈川県（約919万人）に500万人以上の差をつけている。',
      'commentary': '首都圏（東京・神奈川・埼玉・千葉）4都県が上位6位以内に入り、人口の集中がうかがえる。',
      'analysisHeading': '東京都が突出、上位10県のうち首都圏4都県が集中',
      'analysis': [
        '東京都の人口は14,246,219人で、2位神奈川県（9,193,657人）の約1.5倍。3位大阪府（8,764,578人）までの上位3都府県が800万人を超える。',
        '4位愛知県（7,449,403人）・5位埼玉県（7,287,169人）は700万人台で僅差。上位10都道府県のうち首都圏の東京・神奈川・埼玉・千葉の4都県が入っており、10位静岡県（3,468,845人）は1位東京都の4分の1以下の規模になる。',
      ],
      'sections': [],
      'periods': [
        {
          'period': 2024,
          'entries': [
            {
              'name': '東京都',
              'value': 14178000,
              'note': '全国最多の人口を持つ都道府県。',
            },
            {
              'name': '神奈川県',
              'value': 9225000,
              'note': '首都圏に位置し、人口は全国2位。',
            },
            {
              'name': '大阪府',
              'value': 8757000,
              'note': '近畿地方の中心都市を抱え、人口は全国3位。',
            },
          ],
        },
        {
          'period': 2025,
          'entries': [
            {
              'name': '東京都',
              'value': 14246219,
              'note': '全国最多の人口を持つ都道府県。',
            },
            {
              'name': '神奈川県',
              'value': 9193657,
              'note': '首都圏に位置し、人口は全国2位。',
            },
            {
              'name': '大阪府',
              'value': 8764578,
              'note': '近畿地方の中心都市を抱え、人口は全国3位。',
            },
            {
              'name': '愛知県',
              'value': 7449403,
            },
            {
              'name': '埼玉県',
              'value': 7287169,
            },
            {
              'name': '千葉県',
              'value': 6258512,
            },
            {
              'name': '兵庫県',
              'value': 5323825,
            },
            {
              'name': '福岡県',
              'value': 5081879,
            },
            {
              'name': '北海道',
              'value': 4985419,
            },
            {
              'name': '静岡県',
              'value': 3468845,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/jinko-todofuken.webp',
    },
    {
      'id': 'kogyo-shunyu-eiga',
      'title': '年間興行収入ランキング（映画）',
      'category': 'sports',
      'unit': '億円',
      'source': '興行通信社 ほか',
      'sourceUrl': 'https://www.eiren.org/',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-01-01',
      'lead': '2024年の日本の年間興行収入は「名探偵コナン 100万ドルの五稜星（みちしるべ）」が158.0億円で首位。2位「劇場版ハイキュー!! ゴミ捨て場の決戦」（116.4億円）に40億円以上の差をつけている。',
      'commentary': '上位10作品はすべて邦画で、洋画の作品は入っていない。上位2作品は原作漫画・アニメの劇場版が占める。',
      'analysisHeading': '「名探偵コナン」が158.0億円で首位、上位10作品はすべて邦画',
      'analysis': [
        '「名探偵コナン 100万ドルの五稜星（みちしるべ）」は158.0億円で2024年の年間1位。2位「劇場版ハイキュー!! ゴミ捨て場の決戦」（116.4億円）との差は41.6億円で、3位「キングダム 大将軍の帰還」（80.3億円）までが100億円前後の規模になる。',
        '上位10作品は洋画を含まずすべて邦画で、テレビアニメ・漫画を原作とする劇場版が上位10作品中6作品（コナン、ハイキュー、SPY×FAMILY、ガンダムSEED、ドラえもん、ヒロアカ）を占める。',
      ],
      'sections': [],
      'periods': [
        {
          'period': 2023,
          'entries': [
            {
              'name': 'THE FIRST SLAM DUNK',
              'value': 158.7,
            },
            {
              'name': '名探偵コナン 黒鉄の魚影',
              'value': 138.8,
            },
            {
              'name': '君たちはどう生きるか',
              'value': 88.4,
            },
          ],
        },
        {
          'period': 2024,
          'entries': [
            {
              'name': '名探偵コナン 100万ドルの五稜星(みちしるべ)',
              'value': 158,
              'note': '2024年公開のシリーズ最新作で、年間興行収入1位。',
            },
            {
              'name': '劇場版ハイキュー!! ゴミ捨て場の決戦',
              'value': 116.4,
              'note': '2024年公開のアニメ映画で、年間興行収入2位。',
            },
            {
              'name': 'キングダム 大将軍の帰還',
              'value': 80.3,
              'note': '実写映画シリーズの最新作で、年間興行収入3位。',
            },
            {
              'name': '劇場版 SPY×FAMILY CODE: White',
              'value': 63.2,
            },
            {
              'name': 'ラストマイル',
              'value': 59.1,
            },
            {
              'name': '機動戦士ガンダムSEED FREEDOM',
              'value': 53.6,
            },
            {
              'name': '変な家',
              'value': 50.7,
            },
            {
              'name': 'あの花が咲く丘で、君とまた出会えたら。',
              'value': 45.4,
            },
            {
              'name': '映画ドラえもん のび太の地球交響楽',
              'value': 43.1,
            },
            {
              'name': '僕のヒーローアカデミア THE MOVIE ユアネクスト',
              'value': 36,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/kogyo-shunyu-eiga.webp',
    },
    {
      'id': 'kome-shukakuryo-todofuken',
      'title': '都道府県別 米収穫量ランキング',
      'category': 'keizai',
      'unit': 'トン',
      'source': '農林水産省 作物統計',
      'sourceUrl': 'https://www.maff.go.jp/',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-07-14',
      'lead': '2024年産の米収穫量が最も多いのは新潟県で約62.3万トン。2位北海道、3位秋田県と続き、上位は東北・北陸に集中している。全47都道府県を2018年産から2024年産まで掲載している。',
      'commentary': '上位10県のうち5県（秋田・宮城・福島・山形・青森）を東北地方が占め、11位に岩手県が続く。東北6県すべてが上位に並び、日本の米生産の中心となっている。',
      'analysisHeading': '新潟・北海道・秋田が上位、東北地方が生産の中心',
      'analysis': [
        '新潟県の収穫量は622,800トンで、2位北海道（562,400トン）との差は60,400トン。3位秋田県（490,000トン）まで含めた上位3道県が、いずれも40万トンを超える。',
        '上位10県のうち東北地方が5県（秋田・宮城・福島・山形・青森）を占め、11位に岩手県が続く。4位宮城県（366,100トン）から10位青森県（264,200トン）までが密集し、12位富山県（189,000トン）以下は20万トンを下回る。',
        '収穫量の推移を2018年産から2024年産までみると、各県とも作柄や作付面積の影響で年ごとに増減はあるものの、1位新潟県・2位北海道・3位秋田県という上位3道県の並びは7年間一貫している。',
      ],
      'sections': [
        {
          'h2': '上位3道県の7年間の推移（2018〜2024年産）',
        },
        {
          'p': '収穫量の上位3道県は、この7年間**1位新潟県・2位北海道・3位秋田県**の順で固定されている。年ごとの増減はあるが、順位の入れ替わりは一度も起きていない。',
        },
        {
          'table': {
            'headers': [
              '年産',
              '新潟県',
              '北海道',
              '秋田県',
            ],
            'rows': [
              [
                '2018年産',
                '627,600',
                '514,800',
                '491,100',
              ],
              [
                '2019年産',
                '646,100',
                '588,100',
                '526,800',
              ],
              [
                '2020年産',
                '666,800',
                '594,400',
                '527,400',
              ],
              [
                '2021年産',
                '620,000',
                '573,700',
                '501,200',
              ],
              [
                '2022年産',
                '631,000',
                '553,200',
                '456,500',
              ],
              [
                '2023年産',
                '591,700',
                '540,200',
                '458,200',
              ],
              [
                '2024年産',
                '622,800',
                '562,400',
                '490,000',
              ],
            ],
            'caption': '単位: トン。出典: 農林水産省 作物統計',
          },
        },
        {
          'p': '新潟県は2020年産の666,800トンを直近のピークに、2023年産に591,700トンまで減少したのち2024年産で622,800トンへ回復している。北海道・秋田県も同様に年ごとの変動を繰り返しているが、3道県の序列は保たれている。',
        },
        {
          'h2': '上位産地への集中と全国的な減少傾向',
        },
        {
          'p': '2024年産の全国の収穫量に占める割合は、上位3道県（新潟・北海道・秋田）で**22.8%**、上位10県で**53.5%**にのぼる。全国の収穫量は2018年産の約778.0万トンから2024年産の約734.5万トンへ、7年間で5.6%減少している。',
        },
        {
          'p': '増減の傾向は産地の規模で分かれる。上位産地では北海道が7年間で9.2%増と目立って増加し、青森県も0.3%増とほぼ横ばいだった。一方、新潟県（-0.8%）・秋田県（-0.2%）を含む残る45都府県はいずれも減少しており、特に九州・四国・中国地方の中小規模の産地で減少幅が大きい。',
        },
        {
          'h3': '2018年産からの増減率（増加・減少の両端）',
        },
        {
          'table': {
            'headers': [
              '都道府県',
              '2018年産',
              '2024年産',
              '増減率',
            ],
            'rows': [
              [
                '北海道',
                '514,800',
                '562,400',
                '+9.2%',
              ],
              [
                '青森県',
                '263,400',
                '264,200',
                '+0.3%',
              ],
              [
                '新潟県',
                '627,600',
                '622,800',
                '-0.8%',
              ],
              [
                '大分県',
                '103,700',
                '88,400',
                '-14.8%',
              ],
              [
                '山口県',
                '103,400',
                '86,200',
                '-16.6%',
              ],
              [
                '福岡県',
                '182,900',
                '152,200',
                '-16.8%',
              ],
              [
                '香川県',
                '59,900',
                '48,200',
                '-19.5%',
              ],
              [
                '長崎県',
                '57,400',
                '45,900',
                '-20.0%',
              ],
            ],
            'caption': '単位: トン。増減率は2018年産比。出典: 農林水産省 作物統計',
          },
        },
      ],
      'periods': [
        {
          'period': 2018,
          'entries': [
            {
              'name': '新潟県',
              'value': 627600,
              'note': '越後平野を抱える米どころで、収穫量は全国1位。',
            },
            {
              'name': '北海道',
              'value': 514800,
              'note': '広い水田を持ち、収穫量は全国2位。',
            },
            {
              'name': '秋田県',
              'value': 491100,
              'note': 'あきたこまちの産地として知られ、収穫量は全国3位。',
            },
            {
              'name': '山形県',
              'value': 374100,
            },
            {
              'name': '宮城県',
              'value': 371400,
            },
            {
              'name': '福島県',
              'value': 364100,
            },
            {
              'name': '茨城県',
              'value': 358400,
            },
            {
              'name': '栃木県',
              'value': 321800,
            },
            {
              'name': '千葉県',
              'value': 301400,
            },
            {
              'name': '岩手県',
              'value': 273100,
            },
            {
              'name': '青森県',
              'value': 263400,
            },
            {
              'name': '富山県',
              'value': 205900,
            },
            {
              'name': '長野県',
              'value': 199000,
            },
            {
              'name': '福岡県',
              'value': 182900,
            },
            {
              'name': '兵庫県',
              'value': 182000,
            },
            {
              'name': '熊本県',
              'value': 176200,
            },
            {
              'name': '滋賀県',
              'value': 162300,
            },
            {
              'name': '岡山県',
              'value': 156100,
            },
            {
              'name': '埼玉県',
              'value': 155400,
            },
            {
              'name': '愛知県',
              'value': 137700,
            },
            {
              'name': '三重県',
              'value': 137200,
            },
            {
              'name': '福井県',
              'value': 132500,
            },
            {
              'name': '石川県',
              'value': 130300,
            },
            {
              'name': '佐賀県',
              'value': 129300,
            },
            {
              'name': '広島県',
              'value': 122900,
            },
            {
              'name': '岐阜県',
              'value': 107600,
            },
            {
              'name': '大分県',
              'value': 103700,
            },
            {
              'name': '山口県',
              'value': 103400,
            },
            {
              'name': '鹿児島県',
              'value': 92400,
            },
            {
              'name': '島根県',
              'value': 91700,
            },
            {
              'name': '静岡県',
              'value': 79900,
            },
            {
              'name': '宮崎県',
              'value': 79400,
            },
            {
              'name': '群馬県',
              'value': 78900,
            },
            {
              'name': '京都府',
              'value': 72800,
            },
            {
              'name': '愛媛県',
              'value': 69200,
            },
            {
              'name': '鳥取県',
              'value': 63700,
            },
            {
              'name': '香川県',
              'value': 59900,
            },
            {
              'name': '長崎県',
              'value': 57400,
            },
            {
              'name': '徳島県',
              'value': 53600,
            },
            {
              'name': '高知県',
              'value': 50700,
            },
            {
              'name': '奈良県',
              'value': 44100,
            },
            {
              'name': '和歌山県',
              'value': 31600,
            },
            {
              'name': '山梨県',
              'value': 26600,
            },
            {
              'name': '大阪府',
              'value': 24700,
            },
            {
              'name': '神奈川県',
              'value': 15200,
            },
            {
              'name': '沖縄県',
              'value': 2210,
            },
            {
              'name': '東京都',
              'value': 555,
            },
          ],
        },
        {
          'period': 2019,
          'entries': [
            {
              'name': '新潟県',
              'value': 646100,
              'note': '越後平野を抱える米どころで、収穫量は全国1位。',
            },
            {
              'name': '北海道',
              'value': 588100,
              'note': '広い水田を持ち、収穫量は全国2位。',
            },
            {
              'name': '秋田県',
              'value': 526800,
              'note': 'あきたこまちの産地として知られ、収穫量は全国3位。',
            },
            {
              'name': '山形県',
              'value': 404400,
            },
            {
              'name': '宮城県',
              'value': 376900,
            },
            {
              'name': '福島県',
              'value': 368500,
            },
            {
              'name': '茨城県',
              'value': 344200,
            },
            {
              'name': '栃木県',
              'value': 311400,
            },
            {
              'name': '千葉県',
              'value': 289000,
            },
            {
              'name': '青森県',
              'value': 282200,
            },
            {
              'name': '岩手県',
              'value': 279800,
            },
            {
              'name': '富山県',
              'value': 205700,
            },
            {
              'name': '長野県',
              'value': 198400,
            },
            {
              'name': '兵庫県',
              'value': 182900,
            },
            {
              'name': '滋賀県',
              'value': 161400,
            },
            {
              'name': '熊本県',
              'value': 160800,
            },
            {
              'name': '福岡県',
              'value': 158900,
            },
            {
              'name': '岡山県',
              'value': 155600,
            },
            {
              'name': '埼玉県',
              'value': 154200,
            },
            {
              'name': '愛知県',
              'value': 137200,
            },
            {
              'name': '石川県',
              'value': 133000,
            },
            {
              'name': '福井県',
              'value': 130500,
            },
            {
              'name': '三重県',
              'value': 130200,
            },
            {
              'name': '広島県',
              'value': 113300,
            },
            {
              'name': '岐阜県',
              'value': 108500,
            },
            {
              'name': '山口県',
              'value': 91500,
            },
            {
              'name': '大分県',
              'value': 89600,
            },
            {
              'name': '鹿児島県',
              'value': 88500,
            },
            {
              'name': '島根県',
              'value': 87500,
            },
            {
              'name': '静岡県',
              'value': 81200,
            },
            {
              'name': '群馬県',
              'value': 75300,
            },
            {
              'name': '宮崎県',
              'value': 74900,
            },
            {
              'name': '京都府',
              'value': 72700,
            },
            {
              'name': '佐賀県',
              'value': 71800,
            },
            {
              'name': '鳥取県',
              'value': 65300,
            },
            {
              'name': '愛媛県',
              'value': 63900,
            },
            {
              'name': '香川県',
              'value': 56500,
            },
            {
              'name': '徳島県',
              'value': 52400,
            },
            {
              'name': '長崎県',
              'value': 51900,
            },
            {
              'name': '高知県',
              'value': 47900,
            },
            {
              'name': '奈良県',
              'value': 43700,
            },
            {
              'name': '和歌山県',
              'value': 31400,
            },
            {
              'name': '山梨県',
              'value': 26500,
            },
            {
              'name': '大阪府',
              'value': 24300,
            },
            {
              'name': '神奈川県',
              'value': 14300,
            },
            {
              'name': '沖縄県',
              'value': 2000,
            },
            {
              'name': '東京都',
              'value': 519,
            },
          ],
        },
        {
          'period': 2020,
          'entries': [
            {
              'name': '新潟県',
              'value': 666800,
              'note': '越後平野を抱える米どころで、収穫量は全国1位。',
            },
            {
              'name': '北海道',
              'value': 594400,
              'note': '広い水田を持ち、収穫量は全国2位。',
            },
            {
              'name': '秋田県',
              'value': 527400,
              'note': 'あきたこまちの産地として知られ、収穫量は全国3位。',
            },
            {
              'name': '山形県',
              'value': 402400,
            },
            {
              'name': '宮城県',
              'value': 377000,
            },
            {
              'name': '福島県',
              'value': 367000,
            },
            {
              'name': '茨城県',
              'value': 360000,
            },
            {
              'name': '栃木県',
              'value': 318500,
            },
            {
              'name': '千葉県',
              'value': 297500,
            },
            {
              'name': '青森県',
              'value': 283900,
            },
            {
              'name': '岩手県',
              'value': 278700,
            },
            {
              'name': '富山県',
              'value': 206300,
            },
            {
              'name': '長野県',
              'value': 192700,
            },
            {
              'name': '兵庫県',
              'value': 174100,
            },
            {
              'name': '滋賀県',
              'value': 158300,
            },
            {
              'name': '埼玉県',
              'value': 158200,
            },
            {
              'name': '熊本県',
              'value': 156500,
            },
            {
              'name': '岡山県',
              'value': 150500,
            },
            {
              'name': '福岡県',
              'value': 145200,
            },
            {
              'name': '愛知県',
              'value': 134300,
            },
            {
              'name': '石川県',
              'value': 131400,
            },
            {
              'name': '福井県',
              'value': 130000,
            },
            {
              'name': '三重県',
              'value': 129800,
            },
            {
              'name': '広島県',
              'value': 112800,
            },
            {
              'name': '岐阜県',
              'value': 105800,
            },
            {
              'name': '佐賀県',
              'value': 104200,
            },
            {
              'name': '鹿児島県',
              'value': 88400,
            },
            {
              'name': '島根県',
              'value': 87400,
            },
            {
              'name': '大分県',
              'value': 81400,
            },
            {
              'name': '群馬県',
              'value': 76900,
            },
            {
              'name': '宮崎県',
              'value': 76000,
            },
            {
              'name': '静岡県',
              'value': 74100,
            },
            {
              'name': '山口県',
              'value': 73000,
            },
            {
              'name': '京都府',
              'value': 71600,
            },
            {
              'name': '鳥取県',
              'value': 66000,
            },
            {
              'name': '愛媛県',
              'value': 63500,
            },
            {
              'name': '香川県',
              'value': 58000,
            },
            {
              'name': '徳島県',
              'value': 52400,
            },
            {
              'name': '高知県',
              'value': 48900,
            },
            {
              'name': '長崎県',
              'value': 46800,
            },
            {
              'name': '奈良県',
              'value': 40900,
            },
            {
              'name': '和歌山県',
              'value': 28900,
            },
            {
              'name': '山梨県',
              'value': 25800,
            },
            {
              'name': '大阪府',
              'value': 22200,
            },
            {
              'name': '神奈川県',
              'value': 14200,
            },
            {
              'name': '沖縄県',
              'value': 2090,
            },
            {
              'name': '東京都',
              'value': 496,
            },
          ],
        },
        {
          'period': 2021,
          'entries': [
            {
              'name': '新潟県',
              'value': 620000,
              'note': '越後平野を抱える米どころで、収穫量は全国1位。',
            },
            {
              'name': '北海道',
              'value': 573700,
              'note': '広い水田を持ち、収穫量は全国2位。',
            },
            {
              'name': '秋田県',
              'value': 501200,
              'note': 'あきたこまちの産地として知られ、収穫量は全国3位。',
            },
            {
              'name': '山形県',
              'value': 393800,
            },
            {
              'name': '宮城県',
              'value': 353400,
            },
            {
              'name': '茨城県',
              'value': 344800,
            },
            {
              'name': '福島県',
              'value': 335800,
            },
            {
              'name': '栃木県',
              'value': 300900,
            },
            {
              'name': '千葉県',
              'value': 277800,
            },
            {
              'name': '岩手県',
              'value': 268600,
            },
            {
              'name': '青森県',
              'value': 256900,
            },
            {
              'name': '富山県',
              'value': 200000,
            },
            {
              'name': '長野県',
              'value': 189900,
            },
            {
              'name': '兵庫県',
              'value': 175800,
            },
            {
              'name': '福岡県',
              'value': 163700,
            },
            {
              'name': '熊本県',
              'value': 156300,
            },
            {
              'name': '滋賀県',
              'value': 156200,
            },
            {
              'name': '埼玉県',
              'value': 152400,
            },
            {
              'name': '岡山県',
              'value': 150900,
            },
            {
              'name': '愛知県',
              'value': 130900,
            },
            {
              'name': '三重県',
              'value': 130200,
            },
            {
              'name': '福井県',
              'value': 126200,
            },
            {
              'name': '石川県',
              'value': 125400,
            },
            {
              'name': '佐賀県',
              'value': 118800,
            },
            {
              'name': '広島県',
              'value': 115900,
            },
            {
              'name': '岐阜県',
              'value': 103200,
            },
            {
              'name': '大分県',
              'value': 95500,
            },
            {
              'name': '山口県',
              'value': 93100,
            },
            {
              'name': '鹿児島県',
              'value': 89100,
            },
            {
              'name': '島根県',
              'value': 87500,
            },
            {
              'name': '宮崎県',
              'value': 77800,
            },
            {
              'name': '静岡県',
              'value': 77400,
            },
            {
              'name': '群馬県',
              'value': 73300,
            },
            {
              'name': '京都府',
              'value': 71600,
            },
            {
              'name': '愛媛県',
              'value': 67300,
            },
            {
              'name': '鳥取県',
              'value': 63600,
            },
            {
              'name': '香川県',
              'value': 56600,
            },
            {
              'name': '長崎県',
              'value': 50800,
            },
            {
              'name': '高知県',
              'value': 49600,
            },
            {
              'name': '徳島県',
              'value': 47900,
            },
            {
              'name': '奈良県',
              'value': 43200,
            },
            {
              'name': '和歌山県',
              'value': 30300,
            },
            {
              'name': '山梨県',
              'value': 25800,
            },
            {
              'name': '大阪府',
              'value': 22600,
            },
            {
              'name': '神奈川県',
              'value': 14400,
            },
            {
              'name': '沖縄県',
              'value': 2120,
            },
            {
              'name': '東京都',
              'value': 486,
            },
          ],
        },
        {
          'period': 2022,
          'entries': [
            {
              'name': '新潟県',
              'value': 631000,
              'note': '越後平野を抱える米どころで、収穫量は全国1位。',
            },
            {
              'name': '北海道',
              'value': 553200,
              'note': '広い水田を持ち、収穫量は全国2位。',
            },
            {
              'name': '秋田県',
              'value': 456500,
              'note': 'あきたこまちの産地として知られ、収穫量は全国3位。',
            },
            {
              'name': '山形県',
              'value': 365300,
            },
            {
              'name': '宮城県',
              'value': 326500,
            },
            {
              'name': '茨城県',
              'value': 319200,
            },
            {
              'name': '福島県',
              'value': 317300,
            },
            {
              'name': '栃木県',
              'value': 270300,
            },
            {
              'name': '千葉県',
              'value': 259500,
            },
            {
              'name': '岩手県',
              'value': 247600,
            },
            {
              'name': '青森県',
              'value': 235200,
            },
            {
              'name': '富山県',
              'value': 197400,
            },
            {
              'name': '長野県',
              'value': 187300,
            },
            {
              'name': '兵庫県',
              'value': 177000,
            },
            {
              'name': '福岡県',
              'value': 164000,
            },
            {
              'name': '熊本県',
              'value': 156800,
            },
            {
              'name': '滋賀県',
              'value': 151700,
            },
            {
              'name': '岡山県',
              'value': 147200,
            },
            {
              'name': '埼玉県',
              'value': 142400,
            },
            {
              'name': '愛知県',
              'value': 130800,
            },
            {
              'name': '三重県',
              'value': 130800,
            },
            {
              'name': '石川県',
              'value': 122900,
            },
            {
              'name': '福井県',
              'value': 121000,
            },
            {
              'name': '佐賀県',
              'value': 117200,
            },
            {
              'name': '広島県',
              'value': 114500,
            },
            {
              'name': '岐阜県',
              'value': 100800,
            },
            {
              'name': '大分県',
              'value': 93200,
            },
            {
              'name': '山口県',
              'value': 92600,
            },
            {
              'name': '鹿児島県',
              'value': 86000,
            },
            {
              'name': '島根県',
              'value': 85100,
            },
            {
              'name': '静岡県',
              'value': 76400,
            },
            {
              'name': '宮崎県',
              'value': 75200,
            },
            {
              'name': '群馬県',
              'value': 72300,
            },
            {
              'name': '京都府',
              'value': 72000,
            },
            {
              'name': '愛媛県',
              'value': 68600,
            },
            {
              'name': '鳥取県',
              'value': 62200,
            },
            {
              'name': '香川県',
              'value': 55700,
            },
            {
              'name': '高知県',
              'value': 49700,
            },
            {
              'name': '長崎県',
              'value': 48900,
            },
            {
              'name': '徳島県',
              'value': 47600,
            },
            {
              'name': '奈良県',
              'value': 43900,
            },
            {
              'name': '和歌山県',
              'value': 31000,
            },
            {
              'name': '山梨県',
              'value': 25500,
            },
            {
              'name': '大阪府',
              'value': 22800,
            },
            {
              'name': '神奈川県',
              'value': 14400,
            },
            {
              'name': '沖縄県',
              'value': 2000,
            },
            {
              'name': '東京都',
              'value': 484,
            },
          ],
        },
        {
          'period': 2023,
          'entries': [
            {
              'name': '新潟県',
              'value': 591700,
              'note': '越後平野を抱える米どころで、収穫量は全国1位。',
            },
            {
              'name': '北海道',
              'value': 540200,
              'note': '広い水田を持ち、収穫量は全国2位。',
            },
            {
              'name': '秋田県',
              'value': 458200,
              'note': 'あきたこまちの産地として知られ、収穫量は全国3位。',
            },
            {
              'name': '山形県',
              'value': 359300,
            },
            {
              'name': '宮城県',
              'value': 344700,
            },
            {
              'name': '福島県',
              'value': 327600,
            },
            {
              'name': '茨城県',
              'value': 317000,
            },
            {
              'name': '栃木県',
              'value': 284400,
            },
            {
              'name': '千葉県',
              'value': 265700,
            },
            {
              'name': '岩手県',
              'value': 249100,
            },
            {
              'name': '青森県',
              'value': 248700,
            },
            {
              'name': '長野県',
              'value': 187900,
            },
            {
              'name': '富山県',
              'value': 185900,
            },
            {
              'name': '兵庫県',
              'value': 170000,
            },
            {
              'name': '福岡県',
              'value': 157400,
            },
            {
              'name': '熊本県',
              'value': 155400,
            },
            {
              'name': '岡山県',
              'value': 142900,
            },
            {
              'name': '滋賀県',
              'value': 142600,
            },
            {
              'name': '埼玉県',
              'value': 137500,
            },
            {
              'name': '三重県',
              'value': 126300,
            },
            {
              'name': '愛知県',
              'value': 123800,
            },
            {
              'name': '石川県',
              'value': 121200,
            },
            {
              'name': '福井県',
              'value': 116500,
            },
            {
              'name': '佐賀県',
              'value': 115700,
            },
            {
              'name': '広島県',
              'value': 112900,
            },
            {
              'name': '岐阜県',
              'value': 100200,
            },
            {
              'name': '大分県',
              'value': 89900,
            },
            {
              'name': '山口県',
              'value': 87200,
            },
            {
              'name': '鹿児島県',
              'value': 83400,
            },
            {
              'name': '島根県',
              'value': 82900,
            },
            {
              'name': '静岡県',
              'value': 78400,
            },
            {
              'name': '宮崎県',
              'value': 71400,
            },
            {
              'name': '群馬県',
              'value': 70100,
            },
            {
              'name': '京都府',
              'value': 69300,
            },
            {
              'name': '愛媛県',
              'value': 64600,
            },
            {
              'name': '鳥取県',
              'value': 57200,
            },
            {
              'name': '香川県',
              'value': 50600,
            },
            {
              'name': '長崎県',
              'value': 48800,
            },
            {
              'name': '高知県',
              'value': 46800,
            },
            {
              'name': '徳島県',
              'value': 45700,
            },
            {
              'name': '奈良県',
              'value': 42700,
            },
            {
              'name': '和歌山県',
              'value': 29200,
            },
            {
              'name': '山梨県',
              'value': 25700,
            },
            {
              'name': '大阪府',
              'value': 22300,
            },
            {
              'name': '神奈川県',
              'value': 14200,
            },
            {
              'name': '沖縄県',
              'value': 1830,
            },
            {
              'name': '東京都',
              'value': 466,
            },
          ],
        },
        {
          'period': 2024,
          'entries': [
            {
              'name': '新潟県',
              'value': 622800,
              'note': '越後平野を抱える米どころで、収穫量は全国1位。',
            },
            {
              'name': '北海道',
              'value': 562400,
              'note': '広い水田を持ち、収穫量は全国2位。',
            },
            {
              'name': '秋田県',
              'value': 490000,
              'note': 'あきたこまちの産地として知られ、収穫量は全国3位。',
            },
            {
              'name': '宮城県',
              'value': 366100,
            },
            {
              'name': '福島県',
              'value': 356800,
            },
            {
              'name': '山形県',
              'value': 354500,
            },
            {
              'name': '茨城県',
              'value': 338800,
            },
            {
              'name': '千葉県',
              'value': 287900,
            },
            {
              'name': '栃木県',
              'value': 286200,
            },
            {
              'name': '青森県',
              'value': 264200,
            },
            {
              'name': '岩手県',
              'value': 258900,
            },
            {
              'name': '富山県',
              'value': 189000,
            },
            {
              'name': '長野県',
              'value': 187900,
            },
            {
              'name': '兵庫県',
              'value': 166900,
            },
            {
              'name': '熊本県',
              'value': 153300,
            },
            {
              'name': '福岡県',
              'value': 152200,
            },
            {
              'name': '滋賀県',
              'value': 146800,
            },
            {
              'name': '岡山県',
              'value': 142800,
            },
            {
              'name': '埼玉県',
              'value': 140900,
            },
            {
              'name': '愛知県',
              'value': 130200,
            },
            {
              'name': '福井県',
              'value': 124800,
            },
            {
              'name': '三重県',
              'value': 120800,
            },
            {
              'name': '石川県',
              'value': 116200,
            },
            {
              'name': '佐賀県',
              'value': 113100,
            },
            {
              'name': '広島県',
              'value': 108400,
            },
            {
              'name': '岐阜県',
              'value': 101900,
            },
            {
              'name': '大分県',
              'value': 88400,
            },
            {
              'name': '山口県',
              'value': 86200,
            },
            {
              'name': '鹿児島県',
              'value': 80400,
            },
            {
              'name': '島根県',
              'value': 80300,
            },
            {
              'name': '京都府',
              'value': 71700,
            },
            {
              'name': '群馬県',
              'value': 71400,
            },
            {
              'name': '静岡県',
              'value': 70200,
            },
            {
              'name': '宮崎県',
              'value': 69700,
            },
            {
              'name': '愛媛県',
              'value': 63800,
            },
            {
              'name': '鳥取県',
              'value': 59200,
            },
            {
              'name': '香川県',
              'value': 48200,
            },
            {
              'name': '高知県',
              'value': 46700,
            },
            {
              'name': '徳島県',
              'value': 46500,
            },
            {
              'name': '長崎県',
              'value': 45900,
            },
            {
              'name': '奈良県',
              'value': 42100,
            },
            {
              'name': '和歌山県',
              'value': 28700,
            },
            {
              'name': '山梨県',
              'value': 25000,
            },
            {
              'name': '大阪府',
              'value': 20700,
            },
            {
              'name': '神奈川県',
              'value': 13700,
            },
            {
              'name': '沖縄県',
              'value': 1960,
            },
            {
              'name': '東京都',
              'value': 443,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/kome-shukakuryo-todofuken.webp',
    },
    {
      'id': 'nanadairiku-saikoho',
      'title': '七大陸最高峰（大陸別最高峰）ランキング',
      'category': 'chiri',
      'unit': 'm',
      'source': '各種資料',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '七大陸それぞれの最高峰を高さ順に並べると、1位はアジアのエベレスト（8,849m）。2位以下は南米・北米・アフリカと続き、標高差は大きい。',
      'commentary': '七大陸最高峰（セブンサミット）を制覇することは登山家の一つの目標とされる。オセアニアの最高峰には2つの説がある。',
      'analysisHeading': 'エベレストが突出、オセアニア最高峰には2つの説',
      'analysis': [
        'アジアのエベレスト（8,849m）が七大陸最高峰の中でも突出しており、2位南米アコンカグア（6,962m）との差は1,887m。3位北米デナリ（6,190m）、4位アフリカのキリマンジャロ（5,895m）と続く。',
        'オセアニアの最高峰は、ニューギニア島のプンチャック・ジャヤ（4,884m）とする説と、オーストラリア本土のコジオスコ（2,228m）とする説がある。この表では前者を採用している。7つすべての登頂は「セブンサミット」と呼ばれ、登山家の目標の一つとなっている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'エベレスト(アジア)',
              'value': 8849,
              'note': '七大陸最高峰の中で最も標高が高い。',
            },
            {
              'name': 'アコンカグア(南アメリカ)',
              'value': 6962,
              'note': '南アメリカ大陸の最高峰。',
            },
            {
              'name': 'デナリ(北アメリカ)',
              'value': 6190,
              'note': '北アメリカ大陸の最高峰。旧称マッキンリー山。',
            },
            {
              'name': 'キリマンジャロ(アフリカ)',
              'value': 5895,
            },
            {
              'name': 'エルブルス(ヨーロッパ)',
              'value': 5642,
            },
            {
              'name': 'ヴィンソン・マシフ(南極)',
              'value': 4892,
            },
            {
              'name': 'プンチャック・ジャヤ(オセアニア)',
              'value': 4884,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/nanadairiku-saikoho.webp',
    },
    {
      'id': 'natsuki-olympic-kin-medal-kuni',
      'title': '夏季オリンピック金メダル獲得数ランキング（国別・歴代）',
      'category': 'sports',
      'unit': '個',
      'source': 'Wikipedia「近代オリンピックでの国・地域別メダル総獲得数一覧」（パリ2024大会まで集計）',
      'sourceUrl': 'https://ja.wikipedia.org/wiki/%E8%BF%91%E4%BB%A3%E3%82%AA%E3%83%AA%E3%83%B3%E3%83%94%E3%83%83%E3%82%AF%E3%81%A7%E3%81%AE%E5%9B%BD%E3%83%BB%E5%9C%B0%E5%9F%9F%E5%88%A5%E3%83%A1%E3%83%80%E3%83%AB%E7%B7%8F%E7%8D%B2%E5%BE%97%E6%95%B0%E4%B8%80%E8%A6%A7',
      'updateFrequency': 'irregular',
      'updatedAt': '2026-07-12',
      'lead': '夏季オリンピックの歴代金メダル獲得数（パリ2024大会まで）はアメリカが1,101個で世界最多。2位ソビエト連邦（395個、1952〜1988年参加）の約2.8倍にあたる。',
      'commentary': 'ソビエト連邦は1991年に解体しているが、参加した大会（1952〜1988年）の獲得メダルは歴代集計に含まれる。',
      'analysisHeading': 'アメリカが2位ソビエト連邦の約2.8倍で歴代最多',
      'analysis': [
        'アメリカの歴代金メダル獲得数は1,101個で、2位ソビエト連邦（395個）の約2.8倍にあたる。',
        '日本は189個で8位。アジア勢では最上位につけている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'アメリカ',
              'value': 1101,
              'note': '歴代金メダル1,101個で世界最多。2位の約2.8倍の獲得数を持つ。',
            },
            {
              'name': 'ソビエト連邦',
              'value': 395,
              'note': '1952〜1988年に参加し395個の金メダルを獲得。1991年の解体後も歴代集計に含まれる。',
            },
            {
              'name': '中国',
              'value': 303,
              'note': '歴代金メダル303個で世界3位。1984年ロサンゼルス大会から本格参加。',
            },
            {
              'name': 'イギリス',
              'value': 298,
            },
            {
              'name': 'フランス',
              'value': 239,
            },
            {
              'name': 'イタリア',
              'value': 229,
            },
            {
              'name': 'ドイツ',
              'value': 213,
            },
            {
              'name': '日本',
              'value': 189,
            },
            {
              'name': 'ハンガリー',
              'value': 187,
            },
            {
              'name': 'オーストラリア',
              'value': 182,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/natsuki-olympic-kin-medal-kuni.webp',
    },
    {
      'id': 'nihon-koso-biru',
      'title': '日本の高い建造物ランキング',
      'category': 'chiri',
      'unit': 'm',
      'source': '各建築物公表値 ほか',
      'sourceUrl': 'https://www.mlit.go.jp/',
      'updateFrequency': 'irregular',
      'updatedAt': '2026-07-10',
      'lead': '日本一高い建造物は東京スカイツリー（東京都墨田区、634m）。オフィスや住宅が入るビルに限ると、1位は麻布台ヒルズ森JPタワー（325.4m）となる。',
      'commentary': '電波塔のスカイツリー・東京タワーが上位2つを占め、ビルとしての日本一は3位の麻布台ヒルズ森JPタワー。',
      'analysisHeading': '電波塔が上位2つ、ビルとしての1位は麻布台ヒルズ森JPタワー',
      'analysis': [
        '東京スカイツリー（634m）は電波塔で、2位東京タワー（333m）も同じく電波塔。この2つを除くと、居住・業務用のビルとしての最高峰は3位の麻布台ヒルズ森JPタワー（325.4m）になる。',
        '上位10件のうち東京都内の建造物が6件を占める。4位あべのハルカス（300m）、5位横浜ランドマークタワー（296.33m）と、大阪・横浜のビルも上位に入る。8位SiSりんくうタワー（256.1m）から10位虎ノ門ヒルズ森タワー（255.5m）までは、わずか0.6mの範囲に3件がひしめいている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '東京スカイツリー',
              'value': 634,
              'note': '自立式電波塔として世界一の高さを持つ、日本一高い建造物。',
            },
            {
              'name': '東京タワー',
              'value': 333,
              'note': '1958年完成の電波塔で、日本で2番目に高い建造物。',
            },
            {
              'name': '麻布台ヒルズ森JPタワー',
              'value': 325.4,
              'note': '2023年完成の超高層ビルで、日本のビルとして最高の高さを持つ。',
            },
            {
              'name': 'あべのハルカス',
              'value': 300,
            },
            {
              'name': '横浜ランドマークタワー',
              'value': 296.33,
            },
            {
              'name': '虎ノ門ヒルズ ステーションタワー',
              'value': 265.75,
            },
            {
              'name': '麻布台ヒルズ レジデンスB',
              'value': 262.82,
            },
            {
              'name': 'SiSりんくうタワー',
              'value': 256.1,
            },
            {
              'name': '大阪府咲洲庁舎',
              'value': 256,
            },
            {
              'name': '虎ノ門ヒルズ 森タワー',
              'value': 255.5,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/nihon-koso-biru.webp',
    },
    {
      'id': 'nihon-mizuumi-menseki',
      'title': '日本の湖の面積ランキング',
      'category': 'chiri',
      'unit': 'km²',
      'source': '国土地理院 全国都道府県市区町村別面積調 ほか',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '日本最大の湖は琵琶湖（滋賀県）で、面積は約669km²。2位霞ヶ浦の約4倍の広さがあり、日本の湖の中で突出している。上位20湖を掲載している。',
      'commentary': '琵琶湖が単独で突出しており、上位20湖のうち北海道の湖が最も多い。',
      'analysisHeading': '琵琶湖が突出、上位20湖のうち9湖が北海道',
      'analysis': [
        '琵琶湖の面積は669km²で、2位霞ヶ浦（168km²）の約4倍。3位サロマ湖（152km²）以下は面積が近く、2位〜4位は150〜170km²の範囲に収まる。',
        '上位20湖のうち北海道の湖が最も多く、サロマ湖・屈斜路湖・支笏湖・洞爺湖・風蓮湖・能取湖・厚岸湖・網走湖・摩周湖の9湖を占める。20位摩周湖（19km²）は1位琵琶湖の約35分の1の面積になる。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '琵琶湖',
              'value': 669.2,
              'note': '滋賀県にある日本最大の湖。',
            },
            {
              'name': '霞ヶ浦',
              'value': 168.2,
              'note': '茨城県にある日本で2番目に大きい湖。',
            },
            {
              'name': 'サロマ湖',
              'value': 151.6,
              'note': '北海道にある汽水湖で、日本で3番目の面積を持つ。',
            },
            {
              'name': '猪苗代湖',
              'value': 103.2,
            },
            {
              'name': '中海',
              'value': 85.7,
            },
            {
              'name': '屈斜路湖',
              'value': 79.5,
            },
            {
              'name': '宍道湖',
              'value': 79.3,
            },
            {
              'name': '支笏湖',
              'value': 78.5,
            },
            {
              'name': '洞爺湖',
              'value': 70.7,
            },
            {
              'name': '浜名湖',
              'value': 64.9,
            },
            {
              'name': '小川原湖',
              'value': 62.1,
            },
            {
              'name': '十和田湖',
              'value': 61.1,
            },
            {
              'name': '風蓮湖',
              'value': 59,
            },
            {
              'name': '能取湖',
              'value': 58.2,
            },
            {
              'name': '北浦',
              'value': 35,
            },
            {
              'name': '網走湖',
              'value': 32.3,
            },
            {
              'name': '厚岸湖',
              'value': 32.2,
            },
            {
              'name': '八郎潟調整池',
              'value': 27.8,
            },
            {
              'name': '田沢湖',
              'value': 25.8,
            },
            {
              'name': '摩周湖',
              'value': 19.2,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/nihon-mizuumi-menseki.webp',
    },
    {
      'id': 'nihon-nagai-kawa',
      'title': '日本の川の長さランキング',
      'category': 'chiri',
      'unit': 'km',
      'source': '国土交通省',
      'sourceUrl': 'https://www.mlit.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-01-15',
      'lead': '日本最長の川は信濃川。長野県では「千曲川」と呼ばれ、新潟県に入って名前を変える。流域面積で国内最大なのは2位の利根川。',
      'commentary': '信濃川が長野・新潟をまたいで日本最長。2位の利根川は流域面積では国内最大を誇る。',
      'analysisHeading': '信濃川が最長、流域面積では利根川が上回る',
      'analysis': [
        '1位の信濃川は367km、2位の利根川は322kmで、差は45km。利根川の流域面積は日本最大だが、河川の長さでは信濃川が上回る。',
        '上位10河川は北海道（石狩川・天塩川）、東北（北上川・阿武隈川・最上川）、中部（木曽川・天竜川）などに分布する。3位以降は3位石狩川（268km）から10位阿賀野川（210km）までの範囲に収まる。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '信濃川',
              'value': 367,
              'note': '新潟県・長野県を流れる日本最長の川。',
            },
            {
              'name': '利根川',
              'value': 322,
              'note': '関東地方を流れる、日本で2番目に長い川。',
            },
            {
              'name': '石狩川',
              'value': 268,
              'note': '北海道を流れる、日本で3番目に長い川。',
            },
            {
              'name': '天塩川',
              'value': 256,
            },
            {
              'name': '北上川',
              'value': 249,
            },
            {
              'name': '阿武隈川',
              'value': 239,
            },
            {
              'name': '最上川',
              'value': 229,
            },
            {
              'name': '木曽川',
              'value': 227,
            },
            {
              'name': '天竜川',
              'value': 213,
            },
            {
              'name': '阿賀野川',
              'value': 210,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/nihon-nagai-kawa.webp',
    },
    {
      'id': 'nihon-taki-rakusa',
      'title': '日本の滝の落差ランキング',
      'category': 'chiri',
      'unit': 'm',
      'source': '滝ペディア',
      'sourceUrl': 'https://takipedia.com/ranking/rakusa',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-12',
      'lead': '日本一落差が大きい滝は富山県のハンノキ滝で500m。ただし雪解け水が豊富な春から初夏にかけてのみ出現する季節限定の滝で、年間を通じて流れ続ける滝としては同じ称名峡谷の称名滝（350m）が最大となる。',
      'commentary': '上位10件のうち3件が新潟県・富山県に集中しており、日本海側の急峻な地形を反映している。',
      'analysisHeading': 'ハンノキ滝は季節限定、通年で流れる滝としては称名滝が最大',
      'analysis': [
        'ハンノキ滝（富山県）の落差は500mで日本最大だが、雪解け水を水源とする季節限定の滝であり、通年では流れていない。',
        '通年で流れ続ける滝としては、同じ称名峡谷にある称名滝（350m）が日本最大の落差を持つ。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'ハンノキ滝',
              'value': 500,
              'note': '落差500mで日本最大。ただし雪解け水源のため春〜初夏の季節限定で出現する。',
            },
            {
              'name': '称名滝',
              'value': 350,
              'note': '落差350m。通年で流れ続ける滝としては日本最大の落差を持つ。',
            },
            {
              'name': '布晒の滝',
              'value': 300,
              'note': '新潟県にある落差300mの滝。',
            },
            {
              'name': '羽衣の滝',
              'value': 270,
            },
            {
              'name': 'ヌク沢大滝',
              'value': 260,
            },
            {
              'name': '右俣の大滝',
              'value': 250,
            },
            {
              'name': '中の滝',
              'value': 250,
            },
            {
              'name': 'ガンガラシバナの大滝',
              'value': 200,
            },
            {
              'name': 'オロオソロシノ滝',
              'value': 200,
            },
            {
              'name': '梅花皮滝',
              'value': 200,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/nihon-taki-rakusa.webp',
    },
    {
      'id': 'nihon-yama-takasa',
      'title': '日本の山の高さランキング',
      'category': 'chiri',
      'unit': 'm',
      'source': '国土地理院',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '日本一高い山は富士山（3,776m）で、2位北岳との差は583m。上位20座を掲載しており、富士山を除く19座はすべて南アルプス・北アルプスに集中している。',
      'commentary': '富士山だけが単独峰で突出しており、2位以下はすべて南・北アルプスに位置する。',
      'analysisHeading': '富士山が突出、2位以下は南北アルプスに集中',
      'analysis': [
        '富士山の標高は3,776mで、2位北岳（3,193m）との差は583m。2位以下は標高3,000mを超える山が19座続き、南アルプス（北岳・間ノ岳・悪沢岳・赤石岳など）と北アルプス（奥穂高岳・槍ヶ岳・穂高連峰など）が上位を占める。',
        '3位奥穂高岳と4位間ノ岳はいずれも標高3,190mで並ぶ。上位20座のうち富士山を除く19座は、2位北岳（3,193m）から20位立山（3,015m）まで標高差が178mの範囲に収まっている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '富士山',
              'value': 3776,
              'note': '静岡県と山梨県にまたがる日本最高峰。',
            },
            {
              'name': '北岳',
              'value': 3193,
              'note': '南アルプスに位置する、日本で2番目に高い山。',
            },
            {
              'name': '奥穂高岳',
              'value': 3190,
              'note': '北アルプスに位置する日本第3位の高峰。',
            },
            {
              'name': '間ノ岳',
              'value': 3190,
            },
            {
              'name': '槍ヶ岳',
              'value': 3180,
            },
            {
              'name': '悪沢岳',
              'value': 3141,
            },
            {
              'name': '赤石岳',
              'value': 3121,
            },
            {
              'name': '涸沢岳',
              'value': 3110,
            },
            {
              'name': '北穂高岳',
              'value': 3106,
            },
            {
              'name': '大喰岳',
              'value': 3101,
            },
            {
              'name': '前穂高岳',
              'value': 3090,
            },
            {
              'name': '中岳(北アルプス)',
              'value': 3084,
            },
            {
              'name': '中岳(荒川中岳)',
              'value': 3084,
            },
            {
              'name': '御嶽山',
              'value': 3067,
            },
            {
              'name': '農鳥岳',
              'value': 3051,
            },
            {
              'name': '塩見岳',
              'value': 3047,
            },
            {
              'name': '南岳',
              'value': 3033,
            },
            {
              'name': '仙丈ヶ岳',
              'value': 3033,
            },
            {
              'name': '乗鞍岳',
              'value': 3026,
            },
            {
              'name': '立山',
              'value': 3015,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/nihon-yama-takasa.webp',
    },
    {
      'id': 'sekai-coffee-seisanryo-kuni',
      'title': '世界のコーヒー生産量ランキング',
      'category': 'keizai',
      'unit': 'トン',
      'source': 'FAOSTAT（国連食糧農業機関）',
      'sourceUrl': 'https://www.fao.org/faostat/en/#data/QCL',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-13',
      'lead': '世界のコーヒー生豆生産量は、ブラジルが他国を大きく引き離して1位。ベトナム、コロンビアが続く。',
      'commentary': 'FAOSTAT（国連食糧農業機関）の統計に基づく、コーヒー生豆（Green coffee）の生産量ランキング（2020年データ）。',
      'analysisHeading': 'ブラジル1カ国で世界生産量の3割超、上位2カ国で過半数',
      'analysis': [
        'ブラジルの生産量は約370万トンで、2位ベトナム（約176万トン）の2倍以上である。',
        'ブラジルとベトナムの上位2カ国の合計は約546万トンとなり、上位5カ国合計（約654万トン）の8割以上を占める。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'ブラジル',
              'value': 3700847,
            },
            {
              'name': 'ベトナム',
              'value': 1762668,
            },
            {
              'name': 'コロンビア',
              'value': 833930,
            },
            {
              'name': 'インドネシア',
              'value': 762388,
            },
            {
              'name': 'エチオピア',
              'value': 482357,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-coffee-seisanryo-kuni.webp',
    },
    {
      'id': 'sekai-gdp-kuni',
      'title': '世界のGDPランキング（国別）',
      'category': 'keizai',
      'unit': '億ドル',
      'source': 'IMF World Economic Outlook',
      'sourceUrl': 'https://www.imf.org/',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-07-10',
      'lead': '世界最大の経済大国はアメリカで、名目GDPは約323,839億ドル。2位中国（約208,516億ドル）に10兆ドル以上の差をつけている。',
      'commentary': 'GDPの規模では米国・中国の2カ国が突出しており、3位ドイツ以下とは大きな差がある。',
      'analysisHeading': '米国・中国の2カ国が突出、3位以下とは大きな差',
      'analysis': [
        'アメリカの名目GDPは323,839億ドルで、2位中国（208,516億ドル）との差は115,323億ドル。3位ドイツ（54,529億ドル）は中国の4分の1程度の規模になる。',
        '上位10カ国のうちヨーロッパはドイツ・イギリス・フランス・イタリア・ロシアの5カ国、アジアは中国・日本・インドの3カ国、南北アメリカはアメリカ・ブラジルの2カ国。4位日本（43,793億ドル）と6位インド（41,532億ドル）の差は2,261億ドルと小さい。',
      ],
      'sections': [],
      'periods': [
        {
          'period': 2025,
          'entries': [
            {
              'name': 'アメリカ',
              'value': 307670.75,
              'note': '名目GDPで世界最大の経済規模を持つ。',
            },
            {
              'name': '中国',
              'value': 196262.47,
              'note': '名目GDPで世界2位の経済規模を持つ。',
            },
            {
              'name': 'ドイツ',
              'value': 50480.59,
              'note': '名目GDPで世界3位。欧州最大の経済規模を持つ。',
            },
          ],
        },
        {
          'period': 2026,
          'entries': [
            {
              'name': 'アメリカ',
              'value': 323839,
              'note': '名目GDPで世界最大の経済規模を持つ。',
            },
            {
              'name': '中国',
              'value': 208516,
              'note': '名目GDPで世界2位の経済規模を持つ。',
            },
            {
              'name': 'ドイツ',
              'value': 54529,
              'note': '名目GDPで世界3位。欧州最大の経済規模を持つ。',
            },
            {
              'name': '日本',
              'value': 43793,
            },
            {
              'name': 'イギリス',
              'value': 42648,
            },
            {
              'name': 'インド',
              'value': 41532,
            },
            {
              'name': 'フランス',
              'value': 35961,
            },
            {
              'name': 'イタリア',
              'value': 27382,
            },
            {
              'name': 'ロシア',
              'value': 26565,
            },
            {
              'name': 'ブラジル',
              'value': 26359,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-gdp-kuni.webp',
    },
    {
      'id': 'sekai-gengo-washa',
      'title': '世界の言語ランキング（母語話者数）',
      'category': 'jinko',
      'unit': '万人',
      'source': 'Ethnologue',
      'sourceUrl': 'https://www.ethnologue.com/',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-07-10',
      'lead': '母語（第一言語）として話す人が最も多い言語は中国語（標準中国語）で約9.9億人。2位スペイン語の約2倍で、単独で突出している。',
      'commentary': '母語話者数では中国語が最多。英語は第二言語話者を含めると世界最多だが、母語話者だけなら3位となる。',
      'analysisHeading': '母語話者数では中国語が最多、総話者数では英語が逆転',
      'analysis': [
        '標準中国語の母語話者は約9億9,000万人で、2位スペイン語（約4億8,400万人）の約2倍。英語は約3億9,000万人で3位だが、第二言語として話す人を含めた総話者数では世界最多とされる。',
        '上位12言語のうちインド・南アジア系の言語が3つ（ヒンディー語・ベンガル語・西パンジャブ語）を占める。日本語は約1億2,400万人で8位に入っている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': 2025,
          'entries': [
            {
              'name': '中国語(標準)',
              'value': 99000,
              'note': '母語話者数で世界最多の言語。',
            },
            {
              'name': 'スペイン語',
              'value': 48400,
              'note': '中南米・スペインを中心に話され、母語話者数は世界2位。',
            },
            {
              'name': '英語',
              'value': 39000,
              'note': '母語話者数では3位だが、国際的な使用範囲は広い。',
            },
            {
              'name': 'ヒンディー語',
              'value': 34500,
            },
            {
              'name': 'ポルトガル語',
              'value': 25000,
            },
            {
              'name': 'ベンガル語',
              'value': 24200,
            },
            {
              'name': 'ロシア語',
              'value': 14500,
            },
            {
              'name': '日本語',
              'value': 12400,
            },
            {
              'name': '西パンジャブ語',
              'value': 9000,
            },
            {
              'name': 'ベトナム語',
              'value': 8600,
            },
            {
              'name': 'トルコ語',
              'value': 8500,
            },
            {
              'name': '広東語',
              'value': 8500,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-gengo-washa.webp',
    },
    {
      'id': 'sekai-heikinjumyo-kuni',
      'title': '世界の平均寿命ランキング（国別）',
      'category': 'jinko',
      'unit': '歳',
      'source': 'WHO世界保健統計2025年版',
      'sourceUrl': 'https://sekai-hub.com/posts/life-expectancy-ranking-2024',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-07-12',
      'lead': 'WHOが2025年5月に発表した世界保健統計によると、平均寿命が最も長い国は日本で84.5歳。2位シンガポール（83.9歳）を0.6歳上回る。',
      'commentary': '上位10カ国のうち5カ国がヨーロッパ、2カ国がアジアで、いずれも高所得国が占めている。',
      'analysisHeading': '上位10カ国は高所得国が中心、日本がアジア勢で最上位',
      'analysis': [
        '日本の平均寿命は84.5歳で世界1位。2位シンガポール（83.9歳）、3位韓国（83.8歳）とアジア勢が上位に並ぶ。',
        '世界全体の平均寿命は71.4歳であり、上位国は世界平均を10歳以上上回っている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '日本',
              'value': 84.5,
              'note': '平均寿命84.5歳で世界1位。2位以下に0.6歳以上の差をつけている。',
            },
            {
              'name': 'シンガポール',
              'value': 83.9,
              'note': '平均寿命83.9歳で世界2位。アジアで日本に次ぐ長寿国。',
            },
            {
              'name': '韓国',
              'value': 83.8,
              'note': '平均寿命83.8歳で世界3位。上位3カ国はいずれもアジア勢。',
            },
            {
              'name': 'スイス',
              'value': 83.3,
            },
            {
              'name': 'オーストラリア',
              'value': 83.1,
            },
            {
              'name': 'ノルウェー',
              'value': 82.9,
            },
            {
              'name': 'ルクセンブルク',
              'value': 82.8,
            },
            {
              'name': 'スペイン',
              'value': 82.7,
            },
            {
              'name': 'スウェーデン',
              'value': 82.7,
            },
            {
              'name': 'アイスランド',
              'value': 82.6,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-heikinjumyo-kuni.webp',
    },
    {
      'id': 'sekai-jidousha-seisan-kuni',
      'title': '世界の自動車生産台数ランキング（国別）',
      'category': 'keizai',
      'unit': '台',
      'source': 'OICA（国際自動車工業会）',
      'sourceUrl': 'https://www.globalnote.jp/post-3184.html',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-07-12',
      'lead': '2024年の自動車生産台数は中国が3,128万台超で世界最多。2位アメリカの約3倍の規模で、単独で世界生産の3分の1以上を占める。',
      'commentary': '上位10カ国は北米・欧州・アジアに分散しているが、生産規模では中国が他を大きく引き離している。',
      'analysisHeading': '中国が2位アメリカの約3倍、世界生産の3分の1以上を占める',
      'analysis': [
        '中国の自動車生産台数は3,128万1,592台で世界1位。2位アメリカ（1,056万2,188台）の約3倍にあたる。',
        '3位日本（823万4,681台）、4位インド（601万4,691台）と続き、上位4カ国で世界生産の過半数を占める。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '中国',
              'value': 31281592,
              'note': '2024年の生産台数は3,128万台超。2位アメリカの約3倍の規模で世界最大の自動車生産国。',
            },
            {
              'name': 'アメリカ',
              'value': 10562188,
              'note': '生産台数は1,056万台超で世界2位。国内市場向け生産が中心。',
            },
            {
              'name': '日本',
              'value': 8234681,
              'note': '生産台数は823万台超で世界3位。輸出向け生産の比率が高い。',
            },
            {
              'name': 'インド',
              'value': 6014691,
            },
            {
              'name': 'メキシコ',
              'value': 4202642,
            },
            {
              'name': '韓国',
              'value': 4127252,
            },
            {
              'name': 'ドイツ',
              'value': 4069222,
            },
            {
              'name': 'ブラジル',
              'value': 2549595,
            },
            {
              'name': 'スペイン',
              'value': 2376504,
            },
            {
              'name': 'タイ',
              'value': 1468997,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-jidousha-seisan-kuni.webp',
    },
    {
      'id': 'sekai-jinko-kuni',
      'title': '世界の人口ランキング（国別）',
      'category': 'jinko',
      'unit': '人',
      'source': '国連 World Population Prospects ほか',
      'sourceUrl': 'https://population.un.org/wpp/',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-07-10',
      'lead': '世界で最も人口が多い国はインドで、約14億6,390万人。2位中国（約14億1,610万人）をわずかに上回り世界最多となっている。',
      'commentary': 'インド・中国の2カ国だけで世界人口の3分の1近くを占め、3位アメリカとは10億人以上の差がある。',
      'analysisHeading': 'インド・中国の2カ国で世界人口の3分の1近くを占める',
      'analysis': [
        'インドの人口は14億6,390万人で、2位中国（14億1,610万人）との差は4,780万人。3位アメリカ（3億4,730万人）とは10億人以上の開きがあり、上位2カ国が突出している。',
        '上位10カ国のうちアジアが5カ国（インド・中国・インドネシア・パキスタン・バングラデシュ）、アフリカが2カ国（ナイジェリア・エチオピア）を占める。7位ブラジル（2億1,280万人）以下は2億人前後で並ぶ。',
      ],
      'sections': [],
      'periods': [
        {
          'period': 2024,
          'entries': [
            {
              'name': 'インド',
              'value': 1450936000,
              'note': '2023年に中国を上回り、国連推計で世界最多の人口となった。',
            },
            {
              'name': '中国',
              'value': 1408975000,
              'note': '1950年代以降長く世界最多だったが、2023年にインドに首位を譲った。',
            },
            {
              'name': 'アメリカ',
              'value': 340111000,
              'note': '人口世界3位。1位インド・2位中国とは大きな差がある。',
            },
          ],
        },
        {
          'period': 2025,
          'entries': [
            {
              'name': 'インド',
              'value': 1463900000,
              'note': '2023年に中国を上回り、国連推計で世界最多の人口となった。',
            },
            {
              'name': '中国',
              'value': 1416100000,
              'note': '1950年代以降長く世界最多だったが、2023年にインドに首位を譲った。',
            },
            {
              'name': 'アメリカ',
              'value': 347300000,
              'note': '人口世界3位。1位インド・2位中国とは大きな差がある。',
            },
            {
              'name': 'インドネシア',
              'value': 285700000,
            },
            {
              'name': 'パキスタン',
              'value': 255200000,
            },
            {
              'name': 'ナイジェリア',
              'value': 237500000,
            },
            {
              'name': 'ブラジル',
              'value': 212800000,
            },
            {
              'name': 'バングラデシュ',
              'value': 175700000,
            },
            {
              'name': 'ロシア',
              'value': 144000000,
            },
            {
              'name': 'エチオピア',
              'value': 135500000,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-jinko-kuni.webp',
    },
    {
      'id': 'sekai-kaiko-fukasa',
      'title': '世界一深い海溝ランキング',
      'category': 'chiri',
      'unit': 'm',
      'source': '海洋研究開発機構(JAMSTEC) ほか',
      'sourceUrl': 'https://www.jamstec.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-15',
      'lead': '世界一深い海溝はマリアナ海溝で、最深部（チャレンジャー海淵）は10,925m。2位トンガ海溝（10,820m）との差はわずか105mの僅差となっている。',
      'commentary': '海洋観測データに基づく、既知の海溝の最大水深ランキング（上位10海溝）。測定手法により数値に十数m程度の誤差がある。',
      'analysisHeading': '1位マリアナ海溝と2位トンガ海溝の差はわずか105m、3位・4位はさらに僅差の2m',
      'analysis': [
        'マリアナ海溝の最深部（チャレンジャー海淵）は10,925mで、2位トンガ海溝（ホライズン海淵、10,820m）との差はわずか105mしかない。',
        '3位千島・カムチャツカ海溝（10,542m）と4位フィリピン海溝（エンペラー海淵、10,540m）の差はわずか2mで、上位4海溝はいずれも10,500mを超える。5位ケルマデック海溝（10,047m）は4位フィリピン海溝との間に493m以上の差があり、上位4海溝と5位以下の間には明確な差がある。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'マリアナ海溝',
              'value': 10925,
              'note': '太平洋にある世界最深の海溝。最深部はチャレンジャー海淵で、地球上で最も深い地点として知られる。',
            },
            {
              'name': 'トンガ海溝',
              'value': 10820,
              'note': 'マリアナ海溝に次ぐ世界2位の深さを持つが、知名度は低い。',
            },
            {
              'name': '千島・カムチャツカ海溝',
              'value': 10542,
              'note': '千島列島からカムチャツカ半島沖に伸びる海溝。4位フィリピン海溝とはわずか2m差。',
            },
            {
              'name': 'フィリピン海溝',
              'value': 10540,
            },
            {
              'name': 'ケルマデック海溝',
              'value': 10047,
            },
            {
              'name': '伊豆・小笠原海溝',
              'value': 9810,
            },
            {
              'name': 'ニューブリテン海溝',
              'value': 9140,
            },
            {
              'name': '日本海溝',
              'value': 8412,
            },
            {
              'name': 'プエルトリコ海溝',
              'value': 8376,
            },
            {
              'name': 'サウスサンドウィッチ海溝',
              'value': 8266,
            },
          ],
        },
      ],
      'thumbnail': null,
    },
    {
      'id': 'sekai-kaiyo-menseki',
      'title': '世界の海洋 面積ランキング',
      'category': 'chiri',
      'unit': 'km²',
      'source': '国土地理院 ほか',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '世界最大の海洋は太平洋で、面積は約1億8,130万km²。2位大西洋の約1.9倍の広さがあり、四大洋の中で突出している。',
      'commentary': '太平洋1つで四大洋の合計面積の半分近くを占める。',
      'analysisHeading': '太平洋が突出、四大洋のうち北極海が最小',
      'analysis': [
        '太平洋の面積は181,300,000km²で、2位大西洋（94,300,000km²）の約1.9倍。3位インド洋（74,100,000km²）との差は107,200,000km²にのぼる。',
        '4位北極海（12,300,000km²）は太平洋の15分の1程度の広さで、四大洋の中で最も小さい。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '太平洋',
              'value': 181300000,
              'note': '地球上で最も広い海洋。',
            },
            {
              'name': '大西洋',
              'value': 94300000,
              'note': '世界で2番目に広い海洋。',
            },
            {
              'name': 'インド洋',
              'value': 74100000,
              'note': '世界で3番目に広い海洋。',
            },
            {
              'name': '北極海',
              'value': 12300000,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-kaiyo-menseki.webp',
    },
    {
      'id': 'sekai-kawa-nagasa',
      'title': '世界の川の長さランキング',
      'category': 'chiri',
      'unit': 'km',
      'source': '国土地理院 ほか',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '世界最長の川はナイル川で、長さは約6,695km。2位アマゾン川（約6,516km）、3位長江（約6,380km）と続き、上位3河川が6,000kmを超える。',
      'commentary': '上位10河川は大陸ごとに分散し、アジアからは長江・オビ川・エニセイ川・黄河・アムール川の5河川がランクインしている。',
      'analysisHeading': 'ナイル川とアマゾン川は僅差、上位10河川は大陸ごとに分散',
      'analysis': [
        'ナイル川は6,695kmで世界最長。2位アマゾン川（6,516km）との差は179kmと僅差で、川の長さは水源・河口の定義によって数値が変わるため、資料によってはアマゾン川を最長とするものもある。3位長江（6,380km）までの上位3河川が6,000kmを超える。',
        '上位10河川は大陸ごとに分散している。アジアが5河川（長江・オビ川・エニセイ川・黄河・アムール川）、南北アメリカが3河川（アマゾン川・ミシシッピ川・パラナ川）、アフリカが2河川（ナイル川・コンゴ川）を占める。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'ナイル川',
              'value': 6695,
              'note': 'アフリカを流れる世界最長の川。',
            },
            {
              'name': 'アマゾン川',
              'value': 6516,
              'note': '南アメリカを流れ、流域面積は世界最大。長さでは世界2位。',
            },
            {
              'name': '長江',
              'value': 6380,
              'note': 'アジア最長の川で、長さは世界3位。',
            },
            {
              'name': 'ミシシッピ川',
              'value': 5969,
            },
            {
              'name': 'オビ川',
              'value': 5568,
            },
            {
              'name': 'エニセイ川',
              'value': 5550,
            },
            {
              'name': '黄河',
              'value': 5464,
            },
            {
              'name': 'コンゴ川',
              'value': 4667,
            },
            {
              'name': 'パラナ川',
              'value': 4500,
            },
            {
              'name': 'アムール川',
              'value': 4444,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-kawa-nagasa.webp',
    },
    {
      'id': 'sekai-koki-eiga',
      'title': '世界興行収入ランキング（映画・歴代）',
      'category': 'sports',
      'unit': '億ドル',
      'source': 'Box Office Mojo ほか',
      'sourceUrl': 'https://www.boxofficemojo.com/',
      'updateFrequency': 'irregular',
      'updatedAt': '2026-07-10',
      'lead': '歴代世界興行収入で最も稼いだ映画は「アバター」で約29.24億ドル。2位「アベンジャーズ／エンドゲーム」（約27.99億ドル）とは僅差になっている。',
      'commentary': '上位10作品のうち4作品が2020年代公開で、近年の作品も歴代上位に食い込んでいる。',
      'analysisHeading': '「アバター」が歴代1位、上位10作品の4本は2020年代公開',
      'analysis': [
        '「アバター」は約29.24億ドルで歴代1位。2位「アベンジャーズ／エンドゲーム」（約27.99億ドル）との差は1.25億ドルと僅差。3位「アバター：ウェイ・オブ・ウォーター」（約23.35億ドル）まで含めると、上位3作品中2作品を「アバター」シリーズが占める。',
        '上位10作品の公開年を見ると、2020年代の作品が4本（アバター：ウェイ・オブ・ウォーター、ナタ 魔童の大暴れ、スパイダーマン：ノー・ウェイ・ホーム、インサイド・ヘッド2）を占め、1990年代の「タイタニック」（1997年、5位）が唯一の最古参として残っている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'アバター',
              'value': 29.24,
              'note': '2009年公開。歴代興行収入で世界1位を記録している。',
            },
            {
              'name': 'アベンジャーズ/エンドゲーム',
              'value': 27.99,
              'note': '2019年公開のマーベル映画で、歴代興行収入は世界2位。',
            },
            {
              'name': 'アバター:ウェイ・オブ・ウォーター',
              'value': 23.35,
              'note': '2022年公開の続編で、歴代興行収入は世界3位。',
            },
            {
              'name': 'ナタ 魔童の大暴れ',
              'value': 22.67,
            },
            {
              'name': 'タイタニック',
              'value': 22.65,
            },
            {
              'name': 'スター・ウォーズ/フォースの覚醒',
              'value': 20.71,
            },
            {
              'name': 'アベンジャーズ/インフィニティ・ウォー',
              'value': 20.52,
            },
            {
              'name': 'スパイダーマン:ノー・ウェイ・ホーム',
              'value': 19.21,
            },
            {
              'name': 'インサイド・ヘッド2',
              'value': 16.9,
            },
            {
              'name': 'ジュラシック・ワールド',
              'value': 16.7,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-koki-eiga.webp',
    },
    {
      'id': 'sekai-komugi-seisanryo-kuni',
      'title': '世界の小麦生産量ランキング',
      'category': 'keizai',
      'unit': 'トン',
      'source': 'FAOSTAT（国連食糧農業機関）',
      'sourceUrl': 'https://www.fao.org/faostat/en/#data/QCL',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-14',
      'lead': '世界の小麦生産量は中国が突出して世界一。インド、ロシアが僅差で2位・3位を争う。',
      'commentary': 'FAOSTAT（国連食糧農業機関）の統計に基づく、小麦の生産量ランキング（2022年データ）。',
      'analysisHeading': '中国が世界生産量の17%を占め最大生産国、2位インドと3位ロシアの差はわずか350万トン',
      'analysis': [
        '中国の生産量は約1億3770万トンで、世界生産量（約8億840万トン）の17%を占める。',
        '2位インド（約1億770万トン）と3位ロシア（約1億420万トン）の差は約350万トンと僅差である。4位アメリカ合衆国（4490万トン）はロシアの半分以下にとどまる。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '中国',
              'value': 137700000,
            },
            {
              'name': 'インド',
              'value': 107700000,
            },
            {
              'name': 'ロシア',
              'value': 104200000,
            },
            {
              'name': 'アメリカ',
              'value': 44900000,
            },
            {
              'name': 'オーストラリア',
              'value': 36200000,
            },
          ],
        },
      ],
      'thumbnail': null,
    },
    {
      'id': 'sekai-koso-biru',
      'title': '世界の高い建造物ランキング',
      'category': 'chiri',
      'unit': 'm',
      'source': '国土交通省 ほか',
      'sourceUrl': 'https://www.mlit.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '世界一高い建造物はブルジュ・ハリファ（アラブ首長国連邦・ドバイ、高さ828m）。2位メルデカ118（マレーシア、679m）に149mの差をつけている。',
      'commentary': '上位10棟のうち7棟が中国・マレーシア・韓国など東アジア・東南アジアに集中している。',
      'analysisHeading': 'ブルジュ・ハリファが突出、上位10棟の半数を中国が占める',
      'analysis': [
        'ブルジュ・ハリファの高さは828mで、2位メルデカ118（679m）との差は149m。3位上海中心大厦（632m）から10位天津周大福金融中心（530m）までは、隣接する順位間の差が最大42m（6位597m→7位555m）で、それ以外はおおむね30m以内に収まっている。',
        '9位広州周大福金融中心と10位天津周大福金融中心はともに530mで並ぶ。上位10棟のうち中国国内の建造物が上海・深圳・天津・広州の5棟を占め、韓国からはロッテワールドタワー（7位、555m）、マレーシアからはメルデカ118（2位）がランクインしている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'ブルジュ・ハリファ',
              'value': 828,
              'note': 'アラブ首長国連邦・ドバイにある世界最高の超高層ビル。',
            },
            {
              'name': 'メルデカ118',
              'value': 679,
              'note': 'マレーシア・クアラルンプールにある世界2位の高さのビル。',
            },
            {
              'name': '上海中心大厦',
              'value': 632,
              'note': '中国・上海にある世界3位の高さの超高層ビル。',
            },
            {
              'name': 'アブラージュ・アル・バイト時計塔',
              'value': 601,
            },
            {
              'name': '平安国際金融中心',
              'value': 599,
            },
            {
              'name': '高銀金融117',
              'value': 597,
            },
            {
              'name': 'ロッテワールドタワー',
              'value': 555,
            },
            {
              'name': 'ワン・ワールドトレードセンター',
              'value': 541,
            },
            {
              'name': '天津周大福金融中心',
              'value': 530,
            },
            {
              'name': '広州周大福金融中心',
              'value': 530,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-koso-biru.webp',
    },
    {
      'id': 'sekai-kuni-menseki',
      'title': '世界の国土面積ランキング',
      'category': 'chiri',
      'unit': 'km²',
      'source': '外務省',
      'sourceUrl': 'https://www.mofa.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '世界で最も国土面積が広いのはロシアで、約1,710万km²と2位カナダの1.7倍以上。上位50カ国・地域を掲載しており、50位タイでも51万km²を超える。',
      'commentary': 'ロシア1か国で2位カナダとの差が700万km²を超え、上位10カ国の中で突出している。',
      'analysisHeading': 'ロシアが突出、11位以降はアフリカが半数以上',
      'analysis': [
        'ロシアの国土面積は17,098,242km²で、2位カナダ（9,984,670km²）の約1.7倍。1位と2位の差は7,113,572km²で、これは3位アメリカの国土面積（9,833,517km²）に近い大きさになる。',
        '上位10カ国のうち、南北アメリカ大陸の国はカナダ・アメリカ・ブラジル・アルゼンチンの4カ国、アジアは中国・インド・カザフスタンの3カ国。7位インド（3,287,263km²）と8位アルゼンチン（2,780,400km²）の間で面積の差が大きくなり、8位以下は300万km²を下回る。11位から50位までの40カ国のうち、アフリカの国が22カ国と半数以上を占める。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'ロシア',
              'value': 17098242,
              'note': '世界最大の国土面積を持つ国。',
            },
            {
              'name': 'カナダ',
              'value': 9984670,
              'note': '国土面積は世界2位。',
            },
            {
              'name': 'アメリカ',
              'value': 9833517,
              'note': '国土面積は世界3位。',
            },
            {
              'name': '中国',
              'value': 9596961,
            },
            {
              'name': 'ブラジル',
              'value': 8515767,
            },
            {
              'name': 'オーストラリア',
              'value': 7692024,
            },
            {
              'name': 'インド',
              'value': 3287263,
            },
            {
              'name': 'アルゼンチン',
              'value': 2780400,
            },
            {
              'name': 'カザフスタン',
              'value': 2724900,
            },
            {
              'name': 'コンゴ民主共和国',
              'value': 2389986,
            },
            {
              'name': 'アルジェリア',
              'value': 2381741,
            },
            {
              'name': 'サウジアラビア',
              'value': 2149690,
            },
            {
              'name': 'メキシコ',
              'value': 1964375,
            },
            {
              'name': 'インドネシア',
              'value': 1904569,
            },
            {
              'name': 'スーダン',
              'value': 1861484,
            },
            {
              'name': 'リビア',
              'value': 1759540,
            },
            {
              'name': 'イラン',
              'value': 1628750,
            },
            {
              'name': 'モンゴル',
              'value': 1564116,
            },
            {
              'name': 'ペルー',
              'value': 1285216,
            },
            {
              'name': 'チャド',
              'value': 1284000,
            },
            {
              'name': 'ニジェール',
              'value': 1267000,
            },
            {
              'name': 'アンゴラ',
              'value': 1246700,
            },
            {
              'name': 'マリ',
              'value': 1240192,
            },
            {
              'name': '南アフリカ共和国',
              'value': 1221037,
            },
            {
              'name': 'コロンビア',
              'value': 1141748,
            },
            {
              'name': 'エチオピア',
              'value': 1104300,
            },
            {
              'name': 'ボリビア',
              'value': 1098581,
            },
            {
              'name': 'モーリタニア',
              'value': 1030700,
            },
            {
              'name': 'エジプト',
              'value': 1001450,
            },
            {
              'name': 'タンザニア',
              'value': 945087,
            },
            {
              'name': 'ナイジェリア',
              'value': 923768,
            },
            {
              'name': 'ベネズエラ',
              'value': 916445,
            },
            {
              'name': 'パキスタン',
              'value': 881913,
            },
            {
              'name': 'ナミビア',
              'value': 824268,
            },
            {
              'name': 'モザンビーク',
              'value': 801590,
            },
            {
              'name': 'トルコ',
              'value': 783562,
            },
            {
              'name': 'チリ',
              'value': 756102,
            },
            {
              'name': 'ザンビア',
              'value': 752612,
            },
            {
              'name': 'ミャンマー',
              'value': 676886,
            },
            {
              'name': 'アフガニスタン',
              'value': 652864,
            },
            {
              'name': '南スーダン',
              'value': 644329,
            },
            {
              'name': 'ソマリア',
              'value': 637657,
            },
            {
              'name': '中央アフリカ共和国',
              'value': 622984,
            },
            {
              'name': 'ウクライナ',
              'value': 603500,
            },
            {
              'name': 'マダガスカル',
              'value': 587295,
            },
            {
              'name': 'ボツワナ',
              'value': 582000,
            },
            {
              'name': 'ケニア',
              'value': 581313,
            },
            {
              'name': 'フランス',
              'value': 551500,
            },
            {
              'name': 'イエメン',
              'value': 527968,
            },
            {
              'name': 'タイ',
              'value': 513120,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-kuni-menseki.webp',
    },
    {
      'id': 'sekai-mizuumi-menseki',
      'title': '世界の湖の面積ランキング',
      'category': 'chiri',
      'unit': 'km²',
      'source': '国土地理院 ほか',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '世界最大の湖はカスピ海で、面積は約374,000km²と日本の国土面積とほぼ同じ大きさ。2位スペリオル湖の4.5倍以上あり、単独で突出している。',
      'commentary': 'カスピ海は塩湖で、上位10湖のうち淡水湖は9湖。北米の五大湖からはスペリオル湖・ヒューロン湖・ミシガン湖の3湖がランクインしている。',
      'analysisHeading': 'カスピ海が突出、上位10湖のうち9湖は淡水湖',
      'analysis': [
        'カスピ海の面積は374,000km²で、2位スペリオル湖（82,400km²）の4.5倍以上。カスピ海は塩湖で、2位以下の9湖はいずれも淡水湖にあたる。',
        'アフリカからはビクトリア湖（3位、68,800km²）・タンガニーカ湖（6位、32,900km²）・マラウイ湖（9位、29,600km²）の3湖が、北米からはスペリオル湖（2位）・ヒューロン湖（4位、59,600km²）・ミシガン湖（5位、57,800km²）・グレートベア湖（8位、31,000km²）・グレートスレーブ湖（10位、27,200km²）の5湖がランクインしている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'カスピ海',
              'value': 374000,
              'note': '世界最大の湖（塩湖）。',
            },
            {
              'name': 'スペリオル湖',
              'value': 82400,
              'note': '北米五大湖の一つで、世界2位の面積を持つ湖。',
            },
            {
              'name': 'ビクトリア湖',
              'value': 68800,
              'note': 'アフリカ最大の湖で、面積は世界3位。',
            },
            {
              'name': 'ヒューロン湖',
              'value': 59600,
            },
            {
              'name': 'ミシガン湖',
              'value': 57800,
            },
            {
              'name': 'タンガニーカ湖',
              'value': 32900,
            },
            {
              'name': 'バイカル湖',
              'value': 31500,
            },
            {
              'name': 'グレートベア湖',
              'value': 31000,
            },
            {
              'name': 'マラウイ湖',
              'value': 29600,
            },
            {
              'name': 'グレートスレーブ湖',
              'value': 27200,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-mizuumi-menseki.webp',
    },
    {
      'id': 'sekai-sabaku-menseki',
      'title': '世界の砂漠面積ランキング',
      'category': 'chiri',
      'unit': 'km²',
      'source': '各種地理統計（南極・北極の氷雪砂漠を除く）',
      'sourceUrl': 'https://yattoke.com/2018/01/26/desert-ranking/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-12',
      'lead': '世界最大の砂漠はサハラ砂漠で約910万km²。2位アラビア砂漠の約3.5倍の面積を持つ（南極・北極の氷雪砂漠を除く一般的な定義による）。',
      'commentary': '砂漠の定義は「降水量が極端に少なく乾燥した地域」で、南極・北極の氷雪砂漠を含めるかどうかでランキングが変わる。本記事では氷雪砂漠を除いた一般的な定義で集計している。',
      'analysisHeading': 'サハラ砂漠が2位以下を大きく引き離す世界最大の砂漠',
      'analysis': [
        'サハラ砂漠の面積は約910万km²で世界最大。2位アラビア砂漠（約260万km²）の約3.5倍の広さを持つ。',
        '3位ゴビ砂漠（約130万km²）まではアフリカ・中東・アジアの砂漠が並び、4位以下は南半球のパタゴニア・グレートビクトリア砂漠・カラハリ砂漠が続く。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'サハラ砂漠',
              'value': 9100000,
              'note': 'アフリカ大陸北部に広がる世界最大の砂漠。面積は約910万km²。',
            },
            {
              'name': 'アラビア砂漠',
              'value': 2600000,
              'note': 'アラビア半島の大部分を占める世界2位の砂漠。',
            },
            {
              'name': 'ゴビ砂漠',
              'value': 1300000,
              'note': 'モンゴルと中国にまたがる世界3位の砂漠。',
            },
            {
              'name': 'パタゴニア',
              'value': 670000,
            },
            {
              'name': 'グレートビクトリア砂漠',
              'value': 647000,
            },
            {
              'name': 'カラハリ砂漠',
              'value': 570000,
            },
            {
              'name': 'シリア砂漠',
              'value': 490000,
            },
            {
              'name': 'グレートベースン',
              'value': 490000,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-sabaku-menseki.webp',
    },
    {
      'id': 'sekai-shima-menseki',
      'title': '世界の島の面積ランキング',
      'category': 'chiri',
      'unit': 'km²',
      'source': '国土地理院 ほか',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '世界最大の島はグリーンランドで、面積は約2,166,000km²。2位ニューギニア島の2.8倍近くあり、日本の国土面積の5.7倍にあたる。上位30島を掲載しており、日本からは本州（7位）と北海道（21位）がランクインしている。',
      'commentary': '上位10島のうち7位に日本の本州（227,970km²）がランクインしている。',
      'analysisHeading': 'グリーンランドが突出、日本からは本州と北海道がランクイン',
      'analysis': [
        'グリーンランドの面積は2,166,000km²で、2位ニューギニア島（785,753km²）の約2.8倍。ニューギニア島と3位ボルネオ島（743,330km²）の差は42,423km²で、隣接する順位間の差としては小さい部類に入る。',
        '本州は227,970km²で世界の島の中で7位。6位スマトラ島（473,481km²）との差は245,511km²ある一方、8位ヴィクトリア島（217,291km²）との差は10,679km²と僅差になっている。日本の島では北海道（78,073km²）も21位に入り、日本から2島がランクインしている。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'グリーンランド',
              'value': 2166000,
              'note': '世界最大の島（大陸を除く）。',
            },
            {
              'name': 'ニューギニア島',
              'value': 785753,
              'note': 'オセアニアとアジアにまたがる、世界2位の面積を持つ島。',
            },
            {
              'name': 'ボルネオ島',
              'value': 743330,
              'note': 'インドネシア・マレーシア・ブルネイの3カ国にまたがる、世界3位の島。',
            },
            {
              'name': 'マダガスカル島',
              'value': 587041,
            },
            {
              'name': 'バフィン島',
              'value': 507451,
            },
            {
              'name': 'スマトラ島',
              'value': 473481,
            },
            {
              'name': '本州(日本)',
              'value': 227970,
            },
            {
              'name': 'ヴィクトリア島',
              'value': 217291,
            },
            {
              'name': 'グレートブリテン島',
              'value': 209331,
            },
            {
              'name': 'エルズミーア島',
              'value': 196236,
            },
            {
              'name': 'スラウェシ島',
              'value': 174600,
            },
            {
              'name': '南島(ニュージーランド)',
              'value': 151215,
            },
            {
              'name': 'ジャワ島',
              'value': 126700,
            },
            {
              'name': '北島(ニュージーランド)',
              'value': 113729,
            },
            {
              'name': 'ニューファンドランド島',
              'value': 111390,
            },
            {
              'name': 'キューバ島',
              'value': 105007,
            },
            {
              'name': 'ルソン島',
              'value': 104688,
            },
            {
              'name': 'アイスランド島',
              'value': 102828,
            },
            {
              'name': 'ミンダナオ島',
              'value': 94630,
            },
            {
              'name': 'アイルランド島',
              'value': 84406,
            },
            {
              'name': '北海道',
              'value': 78073,
            },
            {
              'name': '樺太',
              'value': 76400,
            },
            {
              'name': 'イスパニョーラ島',
              'value': 74700,
            },
            {
              'name': 'バンクス島',
              'value': 70028,
            },
            {
              'name': 'セイロン島',
              'value': 65268,
            },
            {
              'name': 'タスマニア島',
              'value': 60637,
            },
            {
              'name': 'デヴォン島',
              'value': 55247,
            },
            {
              'name': 'セヴェルヌィ島',
              'value': 48904,
            },
            {
              'name': 'フエゴ島',
              'value': 47992,
            },
            {
              'name': 'バークナー島',
              'value': 43873,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-shima-menseki.webp',
    },
    {
      'id': 'sekai-shukyo-jinko',
      'title': '世界の宗教人口ランキング',
      'category': 'jinko',
      'unit': '億人',
      'source': 'Pew Research Center ほか',
      'sourceUrl': 'https://www.pewresearch.org/',
      'updateFrequency': 'irregular',
      'updatedAt': '2026-07-10',
      'lead': '世界で最も信者が多い宗教はキリスト教で約23.8億人。2位イスラム教（約19.1億人）と合わせ、世界人口の半分以上をこの2宗教が占める。',
      'commentary': 'キリスト教・イスラム教の2宗教で世界人口の過半数を占める。仏教はヒンドゥー教より信者数が少ない。',
      'analysisHeading': 'キリスト教とイスラム教で世界人口の半数以上を占める',
      'analysis': [
        'キリスト教（約23.8億人）とイスラム教（約19.1億人）の2宗教だけで世界人口の半分以上を占める。3位ヒンドゥー教（約11.6億人）は主にインドとその周辺に集中している。',
        '「世界三大宗教」と呼ばれるキリスト教・イスラム教・仏教のうち、仏教（約5.1億人）はヒンドゥー教より信者数が少ない。なお、いずれの宗教にも属さない「無宗教」の人口は約11.9億人と推計され、宗教別に見るとヒンドゥー教と仏教の間に位置する規模になる。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'キリスト教',
              'value': 23.8,
              'note': '世界で最も信者数が多い宗教。',
            },
            {
              'name': 'イスラム教',
              'value': 19.1,
              'note': '世界で2番目に信者数が多い宗教。',
            },
            {
              'name': 'ヒンドゥー教',
              'value': 11.6,
              'note': '主にインドで信仰され、信者数は世界3位。',
            },
            {
              'name': '仏教',
              'value': 5.1,
            },
            {
              'name': '民間信仰・民族宗教',
              'value': 4.3,
            },
            {
              'name': 'その他の宗教',
              'value': 0.6,
            },
            {
              'name': 'ユダヤ教',
              'value': 0.14,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-shukyo-jinko.webp',
    },
    {
      'id': 'sekai-takai-yama',
      'title': '世界の山の高さランキング',
      'category': 'chiri',
      'unit': 'm',
      'source': '国土地理院 ほか',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-01-15',
      'lead': '世界最高峰エベレストを筆頭に、上位10座はすべてヒマラヤ・カラコルム山系が独占。標高8,000mを超える山は世界に14座しかなく、その全座を掲載している。',
      'commentary': '上位10座はすべてヒマラヤ・カラコルム山系。世界の8,000m峰は全部で14座のみ。',
      'analysisHeading': '世界の8,000m峰14座をすべて収録',
      'analysis': [
        '標高8,000mを超える山（8000m峰）は世界に14座しかなく、いずれもヒマラヤ・カラコルム山系に位置する。このランキングは、その14座すべてを収録している。',
        '1位エベレスト（8,849m）と2位K2（8,611m）の差は238m。3位カンチェンジュンガ（8,586m）から10位アンナプルナI峰（8,091m）までは、5位マカルー（8,485m）と6位チョー・オユー（8,188m）の間の297m差を除き、隣接する順位間の差はおおむね100m未満で並ぶ。14座中もっとも標高が低いのは14位シシャパンマ（8,027m）で、1位エベレストとの差は822mになる。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'エベレスト',
              'value': 8849,
              'note': 'ヒマラヤ山脈、ネパールと中国の国境に位置する世界最高峰。',
            },
            {
              'name': 'K2',
              'value': 8611,
              'note': 'カラコルム山脈の最高峰で、世界2位の標高を持つ。',
            },
            {
              'name': 'カンチェンジュンガ',
              'value': 8586,
              'note': 'ネパールとインドの国境に位置する世界3位の高峰。',
            },
            {
              'name': 'ローツェ',
              'value': 8516,
            },
            {
              'name': 'マカルー',
              'value': 8485,
            },
            {
              'name': 'チョー・オユー',
              'value': 8188,
            },
            {
              'name': 'ダウラギリI峰',
              'value': 8167,
            },
            {
              'name': 'マナスル',
              'value': 8163,
            },
            {
              'name': 'ナンガ・パルバット',
              'value': 8126,
            },
            {
              'name': 'アンナプルナI峰',
              'value': 8091,
            },
            {
              'name': 'ガッシャーブルムI峰',
              'value': 8080,
            },
            {
              'name': 'ブロード・ピーク',
              'value': 8051,
            },
            {
              'name': 'ガッシャーブルムII峰',
              'value': 8035,
            },
            {
              'name': 'シシャパンマ',
              'value': 8027,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-takai-yama.webp',
    },
    {
      'id': 'sekai-tosiken-jinko',
      'title': '世界の都市圏人口ランキング',
      'category': 'jinko',
      'unit': '人',
      'source': 'Demographia World Urban Areas',
      'sourceUrl': 'https://www.demographia.com/',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-07-10',
      'lead': '都市圏人口で世界最大は中国の広州＝深圳で、行政区域をまたいだ連続市街地としては約6,956万人。2位上海＝常州（約4,512万人）を大きく引き離している。',
      'commentary': '都市圏の定義（行政区域か連続市街地か）によって順位は変わり、この集計では中国の2都市圏が上位を占める。',
      'analysisHeading': '都市圏の定義次第で順位は変動、中国の2都市圏が上位を独占',
      'analysis': [
        '広州＝深圳の人口は69,562,000人で、2位上海＝常州（45,115,000人）の約1.5倍。この集計は行政区分をまたいでも人口密度400人/km²以上で市街地が連続していれば1つの都市圏として数える基準（Demographia方式）を採用しており、複数の行政区域にまたがる中国の大都市圏が上位に来やすい。',
        '3位東京＝横浜（37,325,000人）、4位ジャカルタ（36,877,000人）、5位デリー（33,224,000人）と続く。上位10都市圏のうちインドが2都市圏（デリー・ムンバイ）、東南・南アジアが3都市圏（ジャカルタ・マニラ・ダッカ）を占める。',
      ],
      'sections': [],
      'periods': [
        {
          'period': 2025,
          'entries': [
            {
              'name': '広州=深圳',
              'value': 69562000,
              'note': '中国広東省に位置する、人口規模が世界最大級の都市圏。',
            },
            {
              'name': '上海=常州',
              'value': 45115000,
              'note': '中国有数の巨大都市圏。',
            },
            {
              'name': '東京=横浜',
              'value': 37325000,
              'note': '日本の首都圏を含む、世界最大級の都市圏の一つ。',
            },
            {
              'name': 'ジャカルタ',
              'value': 36877000,
            },
            {
              'name': 'デリー',
              'value': 33224000,
            },
            {
              'name': 'ムンバイ',
              'value': 26237000,
            },
            {
              'name': 'マニラ',
              'value': 25521000,
            },
            {
              'name': 'ダッカ',
              'value': 25305000,
            },
            {
              'name': 'ソウル=仁川',
              'value': 23825000,
            },
            {
              'name': 'カイロ',
              'value': 22684000,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/sekai-tosiken-jinko.webp',
    },
    {
      'id': 'taiyokei-wakusei-chokkei',
      'title': '太陽系の惑星サイズランキング',
      'category': 'chiri',
      'unit': 'km',
      'source': 'NASA Planetary Fact Sheet',
      'sourceUrl': 'https://nssdc.gsfc.nasa.gov/planetary/factsheet/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-15',
      'lead': '太陽系で最も直径が大きい惑星は木星で約142,984km。2位土星（約120,536km）を2万km以上上回る。',
      'commentary': 'NASA Planetary Fact Sheetの赤道直径データに基づく、太陽系8惑星のサイズ（直径）ランキング。',
      'analysisHeading': '木星が地球の約11倍で太陽系最大、上位4惑星(ガス惑星)と下位4惑星(地球型惑星)の間には3.6万kmの差',
      'analysis': [
        '木星の直径は約142,984kmで、太陽系最小の水星（約4,879km）の29倍以上、地球（約12,756km）の約11倍にあたる。',
        '直径上位4惑星（木星・土星・天王星・海王星）はいずれもガス惑星で、4位海王星（約49,528km）と5位地球（約12,756km）の差は36,772kmある。地球型惑星の最大である地球でも、ガス惑星最小の天王星（約51,118km）の4分の1に満たない。',
        '参考として月（直径約3,474km）と冥王星（直径約2,377km）を8惑星と合わせて直径順に並べると、月は8位水星（約4,879km）に次いで9位、冥王星は10位でこの10天体中最小となる。ただし月は惑星ではなく地球の衛星、冥王星は2006年のIAU総会以降「準惑星」に分類されており、いずれも本ランキングの8惑星には含まれない。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '木星',
              'value': 142984,
            },
            {
              'name': '土星',
              'value': 120536,
            },
            {
              'name': '天王星',
              'value': 51118,
            },
            {
              'name': '海王星',
              'value': 49528,
            },
            {
              'name': '地球',
              'value': 12756,
            },
            {
              'name': '金星',
              'value': 12104,
            },
            {
              'name': '火星',
              'value': 6792,
            },
            {
              'name': '水星',
              'value': 4879,
            },
          ],
        },
      ],
      'thumbnail': null,
    },
    {
      'id': 'todofuken-menseki',
      'title': '都道府県別 面積ランキング',
      'category': 'chiri',
      'unit': 'km²',
      'source': '国土地理院 全国都道府県市区町村別面積調',
      'sourceUrl': 'https://www.gsi.go.jp/',
      'updateFrequency': 'static',
      'updatedAt': '2026-07-10',
      'lead': '都道府県別の面積で最も広いのは北海道（約83,424km²）。2位岩手県の約5.5倍にあたり、上位10県は東北・中部地方が中心となっている。',
      'commentary': '上位10県のうち東北地方が5県を占め、北海道が全体で突出して広い。',
      'analysisHeading': '北海道が突出、上位10県のうち5県が東北地方',
      'analysis': [
        '北海道の面積は83,424km²で、2位岩手県（15,275km²）の約5.5倍、10位鹿児島県（9,186km²）の約9倍にあたる。上位10県のうち、岩手県・福島県・秋田県・青森県・山形県の5県を東北地方が占める。',
        '3位福島県（13,784km²）・4位長野県（13,562km²）・5位新潟県（12,584km²）は差が1,000km²前後で並ぶ。最も面積が狭い都道府県は香川県（1,877km²）で、1位北海道との差は約81,500km²になる。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': '北海道',
              'value': 83424,
              'note': '都道府県別面積で全国最大。',
            },
            {
              'name': '岩手県',
              'value': 15275,
              'note': '本州最大の面積を持つ県で、都道府県別では全国2位。',
            },
            {
              'name': '福島県',
              'value': 13784,
              'note': '東北地方に位置し、都道府県別面積は全国3位。',
            },
            {
              'name': '長野県',
              'value': 13562,
            },
            {
              'name': '新潟県',
              'value': 12584,
            },
            {
              'name': '秋田県',
              'value': 11638,
            },
            {
              'name': '岐阜県',
              'value': 10621,
            },
            {
              'name': '青森県',
              'value': 9646,
            },
            {
              'name': '山形県',
              'value': 9323,
            },
            {
              'name': '鹿児島県',
              'value': 9186,
            },
            {
              'name': '広島県',
              'value': 8478,
            },
            {
              'name': '兵庫県',
              'value': 8401,
            },
            {
              'name': '静岡県',
              'value': 7777,
            },
            {
              'name': '宮崎県',
              'value': 7734,
            },
            {
              'name': '熊本県',
              'value': 7409,
            },
            {
              'name': '宮城県',
              'value': 7282,
            },
            {
              'name': '岡山県',
              'value': 7114,
            },
            {
              'name': '高知県',
              'value': 7102,
            },
            {
              'name': '島根県',
              'value': 6708,
            },
            {
              'name': '栃木県',
              'value': 6408,
            },
            {
              'name': '群馬県',
              'value': 6362,
            },
            {
              'name': '大分県',
              'value': 6341,
            },
            {
              'name': '山口県',
              'value': 6113,
            },
            {
              'name': '茨城県',
              'value': 6098,
            },
            {
              'name': '三重県',
              'value': 5774,
            },
            {
              'name': '愛媛県',
              'value': 5676,
            },
            {
              'name': '愛知県',
              'value': 5173,
            },
            {
              'name': '千葉県',
              'value': 5156,
            },
            {
              'name': '福岡県',
              'value': 4988,
            },
            {
              'name': '和歌山県',
              'value': 4725,
            },
            {
              'name': '京都府',
              'value': 4612,
            },
            {
              'name': '山梨県',
              'value': 4465,
            },
            {
              'name': '富山県',
              'value': 4248,
            },
            {
              'name': '石川県',
              'value': 4191,
            },
            {
              'name': '福井県',
              'value': 4191,
            },
            {
              'name': '徳島県',
              'value': 4147,
            },
            {
              'name': '長崎県',
              'value': 4131,
            },
            {
              'name': '滋賀県',
              'value': 4017,
            },
            {
              'name': '埼玉県',
              'value': 3798,
            },
            {
              'name': '奈良県',
              'value': 3691,
            },
            {
              'name': '鳥取県',
              'value': 3507,
            },
            {
              'name': '佐賀県',
              'value': 2441,
            },
            {
              'name': '神奈川県',
              'value': 2417,
            },
            {
              'name': '沖縄県',
              'value': 2282,
            },
            {
              'name': '東京都',
              'value': 2200,
            },
            {
              'name': '大阪府',
              'value': 1905,
            },
            {
              'name': '香川県',
              'value': 1877,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/todofuken-menseki.webp',
    },
    {
      'id': 'wcup-yusho-kaisu',
      'title': 'FIFAワールドカップ優勝回数ランキング（国別）',
      'category': 'sports',
      'unit': '回',
      'source': 'FIFA',
      'sourceUrl': 'https://www.fifa.com/',
      'updateFrequency': 'irregular',
      'updatedAt': '2022-12-18',
      'lead': 'FIFAワールドカップ優勝回数が最も多いのはブラジルで5回。2位はドイツ・イタリアが4回で並ぶ。優勝経験があるのは史上8カ国のみ。',
      'commentary': '直近の優勝国は2022年カタール大会のアルゼンチン。2026年大会は開催中のため、この結果にはまだ反映されていない。',
      'analysisHeading': '優勝経験があるのは史上8カ国のみ、ブラジルが最多5回',
      'analysis': [
        '優勝経験があるのは史上8カ国のみ。ブラジルが5回で最多、2位はドイツ・イタリアが4回で並ぶ。4位アルゼンチンは3回、5位はウルグアイ・フランスが2回で並ぶ。',
        '直近の優勝国は2022年カタール大会のアルゼンチン。2026年大会（アメリカ・カナダ・メキシコ共催、48チームに拡大）は開催中のため、この結果にはまだ反映されていない。',
      ],
      'sections': [],
      'periods': [
        {
          'period': null,
          'entries': [
            {
              'name': 'ブラジル',
              'value': 5,
              'note': 'ワールドカップ最多となる5回の優勝を誇る。',
            },
            {
              'name': 'ドイツ',
              'value': 4,
              'note': '優勝4回でイタリアと並び歴代2位。',
            },
            {
              'name': 'イタリア',
              'value': 4,
              'note': '優勝4回でドイツと並び歴代2位。',
            },
            {
              'name': 'アルゼンチン',
              'value': 3,
            },
            {
              'name': 'フランス',
              'value': 2,
            },
            {
              'name': 'ウルグアイ',
              'value': 2,
            },
            {
              'name': 'スペイン',
              'value': 1,
            },
            {
              'name': 'イングランド',
              'value': 1,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/wcup-yusho-kaisu.webp',
    },
    {
      'id': 'yushutsugaku-kuni',
      'title': '国・地域別 輸出額ランキング',
      'category': 'keizai',
      'unit': '億ドル',
      'source': 'UNCTAD 貿易統計',
      'sourceUrl': 'https://unctad.org/',
      'updateFrequency': 'yearly',
      'updatedAt': '2026-01-01',
      'lead': '2024年の輸出額（財貨・サービス）は中国が1位で3,576.65億ドル。2位アメリカ（2,065.13億ドル）に1,500億ドル以上の差をつけている。',
      'commentary': '中国・アメリカ・ドイツの上位3カ国が突出しており、4位以下とは規模の差が大きい。',
      'analysisHeading': '中国が突出、上位3カ国と4位以下の差が大きい',
      'analysis': [
        '中国の輸出額は3,576.65億ドルで、2位アメリカ（2,065.13億ドル）の約1.7倍。3位ドイツ（1,681.66億ドル）まで含めた上位3カ国が1,500億ドルを超え、4位オランダ（921.22億ドル）以下とは規模の差が大きい。',
        '5位日本（707.01億ドル）から10位メキシコ（617.10億ドル）までは600〜700億ドル台に収まり、僅差の争いになっている。上位10カ国・地域のうちアジアが4カ国（中国・日本・韓国・香港）を占める。',
      ],
      'sections': [],
      'periods': [
        {
          'period': 2024,
          'entries': [
            {
              'name': '中国',
              'value': 3576.65,
              'note': '財貨・サービスの輸出額で世界最大。',
            },
            {
              'name': 'アメリカ',
              'value': 2065.13,
              'note': '輸出額で世界2位。',
            },
            {
              'name': 'ドイツ',
              'value': 1681.66,
              'note': '欧州最大の輸出国で、輸出額は世界3位。',
            },
            {
              'name': 'オランダ',
              'value': 921.22,
            },
            {
              'name': '日本',
              'value': 707.01,
            },
            {
              'name': '韓国',
              'value': 683.61,
            },
            {
              'name': 'イタリア',
              'value': 674.46,
            },
            {
              'name': '香港',
              'value': 645.54,
            },
            {
              'name': 'フランス',
              'value': 639.05,
            },
            {
              'name': 'メキシコ',
              'value': 617.1,
            },
          ],
        },
      ],
      'thumbnail': '/images/topics/yushutsugaku-kuni.webp',
    },
  ],
};
